...
export const UniversalPreviewFrame: React.FC<UniversalPreviewFrameProps> = ({
        ...
        onclone: (clonedDoc: Document, clonedEl: HTMLElement) => {
          try {
            ...
            document
              ...
              .forEach((l) => clonedDoc.head.appendChild(l.cloneNode(true)));
            document
              .querySelectorAll('style')
              .forEach((s) => clonedDoc.head.appendChild(s.cloneNode(true)));
  ...
  const handleFitToPage = useCallback(() => {
    ...
    // Find the first worksheet page to get its dimensions
    const firstPage = container.querySelector('.worksheet-page') as HTMLElement;
    const containerWidth = container.clientWidth - 64; // padding

    let pageWidthPx = 210 * 3.7795; // default A4 portrait
    if (firstPage) {
      pageWidthPx = firstPage.offsetWidth;
    }
  ...
  return (
    <div
      className={`flex-1 ${bgClass} h-full relative flex flex-col items-center overflow-hidden transition-all duration-500`}
    >
      {/* ═══ ÜST ARAÇ ÇUBUĞU ═══ */}
            ...
            {!downloadLink && printSelector && mode === 'html' && (
              <>
                ...
                >
                  ...
                  Dışa Aktar
                  <i
                    className={`fa-solid fa-chevron-down text-[8px] text-slate-400 transition-transform ${exportOpen ? 'rotate-180' : ''}`}
                  ></i>
                </button>
                  ...
                  <div className="absolute top-full mt-1.5 right-0 bg-white rounded-2xl shadow-2xl border border-slate-200 p-1.5 min-w-[200px] z-50 animate-in fade-in zoom-in-95 duration-150">
                    ...
                    >
                      <div className="w-7 h-7 rounded-lg bg-amber-100 flex items-center justify-center">
                        <i
                          className={`fa-solid ${copySuccess ? 'fa-check' : 'fa-copy'} text-amber-600 text-[10px]`}
                        ></i>
                      </div>
                      <div className="text-left">
