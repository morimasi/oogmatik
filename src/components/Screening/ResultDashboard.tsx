    ...
    try {
      ...
      await Promise.all(
        receiverIds.map((receiverId) =>
          assessmentService.shareAssessment(data, user.id, user.name, receiverId)
        )
      );
      setIsSharing(false);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="print-page relative bg-white h-[297mm] w-[210mm] p-12 flex flex-col font-sans text-black">
          ...
          <div className="flex-1 space-y-10">
            ...
            <div className="bg-zinc-50 p-6 rounded-2xl border-2 border-zinc-200">
              <h3 className="font-black text-sm uppercase mb-3 border-b border-zinc-300 pb-2">
                Genel Değerlendirme
              </h3>
              ...
              <tbody>
                {Object.entries(result.categoryScores).map(([cat, data], i) => (
                  <tr key={cat} className={i % 2 === 0 ? 'bg-white' : 'bg-zinc-50'}>
                    ...
                    <td className="p-3 border-r border-zinc-200 text-center">
          ...
          <div className="flex-1 space-y-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-2">
                {Object.values(result.categoryScores)
                  .flatMap((s) => (s as any).findings)
                ...
                {Object.values(result.categoryScores).flatMap((s) => (s as any).findings).length ===
                  0 && (
                  <p className="text-sm italic text-zinc-500">
                    Belirgin bir risk bulgusu kaydedilmedi.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="mt-8 p-4 border border-dashed border-zinc-300 rounded-xl text-xs text-zinc-500 text-center">
              <p>
                ...
                <strong>{result.respondentRole === 'parent' ? 'Ebeveyn' : 'Öğretmen'}</strong>
              </p>
      ...
      <ShareModal
        ...
        onShare={handleShare}
        worksheetTitle={`Tarama Raporu: ${result.studentName}`}
      />
    </div>
  );
};
