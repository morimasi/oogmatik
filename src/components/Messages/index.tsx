...
export const MessagingModule: React.FC = () => {
  ...
  return (
    <div className="flex h-full bg-[var(--bg-default)] text-[var(--text-primary)] overflow-hidden font-lexend relative">
      <ToastNotification />
      <div className={`${showMobileList ? 'flex' : 'hidden'} md:flex w-full md:w-[280px] border-r border-[var(--border-color)] bg-[var(--bg-paper)] absolute md:relative inset-0 z-30 md:z-auto`}>
        <ConversationList />
      </div>
      <div className={`${!showMobileList ? 'flex' : 'hidden'} md:flex flex-1 relative bg-[var(--bg-default)]`}>
        {isArchiveOpen ? <ArchivePanel onClose={() => setIsArchiveOpen(false)} /> : (
          <>
            <ChatWindow />
            {canViewArchive && (
              <button onClick={() => setIsArchiveOpen(true)}
                className="hidden md:flex absolute top-3 right-16 z-20 p-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 border border-rose-500/30 rounded-lg transition-all shadow-sm backdrop-blur-md items-center gap-1.5 group">
      ...
      <AnimatePresence>
        {activeThreadId && (
          <motion.div initial={{ x: '100%', opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: '100%', opacity: 0 }}
            ...
            className="hidden md:flex w-[320px] border-l border-[var(--border-color)] bg-[var(--bg-paper)] shadow-xl z-20">
            <ThreadPanel />
          </motion.div>
        )}
      </AnimatePresence>
