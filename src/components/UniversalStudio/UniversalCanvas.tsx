...
const DraggableItem = ({ item, children, canvasWidth }: DraggableItemProps) => {
  const {
    designMode,
    updateComponent,
    selectedId,
  ...
  return (
    <div
      ...
      style={{
        ...
        height:
          item.id === 'activity_component' && !item.groupId ? 'auto' : (item.style.h as number),
        minHeight:
          item.id === 'activity_component' && !item.groupId ? `${item.style.h}px` : undefined,
    ...
    >
      ...
      {designMode && isSelected && (
        <>
          <div className="absolute -top-10 left-0 bg-indigo-600 text-white text-[10px] font-black px-3 py-2 rounded-lg shadow-xl uppercase tracking-wider flex items-center gap-2 z-50">
            ...
            <button
              onClick={(e: React.MouseEvent) => {
                e.stopPropagation();
                updateComponent(item.instanceId, { isVisible: false });
              }}
          ...
          <div
            className={`absolute inset-0 ring-2 ${isLocked ? 'ring-gray-400 ring-offset-1' : 'ring-indigo-500 ring-offset-2'} pointer-events-none rounded-sm`}
          ></div>
        </>
      )}
...
export const UniversalCanvas = ({ settings }: { settings?: any }) => {
    ...
    if (item.id === 'header') {
      return (
        <div style={boxStyle} className="h-full flex flex-col justify-center">
          <h2 className="text-xl font-bold uppercase border-b-2 border-zinc-200 pb-1">
            {item.specificData.title as any}
          </h2>
        </div>
      );
    }
    ...
    return (
      <div style={boxStyle} className="h-full w-full">
        <BlockRenderer
          block={{
            type: item.id as any,
            content: item.specificData.content as any,
            style: {} as any,
          }}
        />
      </div>
  ...
  return (
    <div className="flex flex-col gap-8 w-full items-center pb-20">
      ...
      {Array.from({
        length: Math.max(1, ...layout.map((l: LayoutItem) => (l.pageIndex || 0) + 1)),
      }).map((_: any, pageIndex: number) => {
        const pageItems = layout.filter(
          (l: LayoutItem) => l.isVisible && (l.pageIndex || 0) === pageIndex
        );
