...
export const DrillSettingsPanel: React.FC<DrillSettingsPanelProps> = ({ drillConfig, setDrillConfig, toggleDrillOp }) => (
    <div className="p-5 space-y-6 animate-in slide-in-from-left-4">
        ...
        <div className="space-y-2 p-4 bg-zinc-900 rounded-2xl border border-zinc-800">
            ...
            {drillConfig.selectedOperations.includes('add') && (
                <Toggle label="Toplama: Eldeli" checked={drillConfig.allowCarry} onChange={v => setDrillConfig({ ...drillConfig, allowCarry: v })} />
            )}

            {drillConfig.selectedOperations.includes('sub') && (
                <>
                    <Toggle label="Çıkarma: Onluk Bozma" checked={drillConfig.allowBorrow} onChange={v => setDrillConfig({ ...drillConfig, allowBorrow: v })} />
                    <Toggle label="Çıkarma: Negatif" checked={drillConfig.allowNegative} onChange={v => setDrillConfig({ ...drillConfig, allowNegative: v })} />
                </>
            )}

            {drillConfig.selectedOperations.includes('div') && (
                <Toggle label="Bölme: Kalanlı" checked={drillConfig.allowRemainder} onChange={v => setDrillConfig({ ...drillConfig, allowRemainder: v })} />
            )}
        </div>
        ...
        <div className="space-y-2">
            ...
            {[
                { label: 'Sayfayı Otomatik Doldur', key: 'autoFillPage' as const },
