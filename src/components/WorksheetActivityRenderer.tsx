  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={{ display: 'flex', gap: '16px', marginBottom: '20px', fontSize: '13px' }}>
        <div style={{ flex: 1 }}>Ad Soyad: <span style={blankLineStyle as React.CSSProperties}>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></div>
        <div>Tarih: <span style={blankLineStyle as React.CSSProperties}>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></div>
      </div>
      ...
      {showAnswerKey && data.hasAnswerKey && (
        <div style={{ marginTop: '32px', borderTop: '2px dashed #94a3b8', paddingTop: '16px' }}>
          <div style={{ fontSize: '16px', fontWeight: 700, marginBottom: '12px', color: '#6366f1' }}>
            🔑 Cevap Anahtarı
          </div>
          {data.sections.filter(s => s.correctAnswer).map((s) => (
            <div key={s.id} style={{ fontSize: '13px', marginBottom: '4px', color: '#334155' }}>
              <strong>{s.order}.</strong> {Array.isArray(s.correctAnswer)
...
const SectionRenderer: React.FC<{ section: WorksheetSection; showAnswer: boolean }> = ({ section, showAnswer }) => {
  return (
    <div style={sectionStyle}>
      {/* Soru Numarası + Yönerge */}
      <div style={{ fontSize: '14px', fontWeight: 600, color: '#1e293b', marginBottom: '6px' }}>
        <span style={{ color: '#6366f1', fontWeight: 700, marginRight: '8px' }}>{section.order}.</span>
        {section.instruction}
      </div>

      {/* İçerik */}
      {section.content && (
        <div style={{ fontSize: '14px', color: '#334155', marginBottom: '8px', lineHeight: 1.9 }}>
          {section.content}
        </div>
      )}
...
const AnswerAreaRenderer: React.FC<{ section: WorksheetSection; showAnswer: boolean }> = ({ section, showAnswer }) => {
  const { answerArea } = section;

  switch (answerArea.type) {
    case 'blank-line':
      return <BlankLineRenderer lines={answerArea.lines ?? 2} answer={showAnswer ? section.correctAnswer : undefined} />;
    ...
    case 'multiple-choice':
      return <MultipleChoiceRenderer options={section.options} answer={showAnswer ? section.correctAnswer : undefined} />;
...
const BlankLineRenderer: React.FC<{ lines: number; answer?: string | string[] }> = ({ lines, answer }) => (
  <div>
    {Array.from({ length: lines }).map((_, i) => (
      <div key={i} style={{ ...blankLineStyle, position: 'relative' }}>
        {answer && i === 0 && (
          <span style={{ position: 'absolute', top: 2, left: 4, color: '#16a34a', fontSize: '12px', fontStyle: 'italic' }}>
...
const BlankBoxRenderer: React.FC<{ answer?: string | string[] }> = ({ answer }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexWrap: 'wrap', marginTop: '4px' }}>
    ...
    <span style={blankBoxStyle}>
      {answer ? (
        <span style={{ color: '#16a34a' }}>{Array.isArray(answer) ? answer.join(', ') : answer}</span>
      ) : ''}
    </span>
  </div>
);
...
const GridRenderer: React.FC<{
  ...
  const rows = gridData ?? Array.from({ length: gridSize?.rows ?? 4 }, () =>
    Array.from({ length: gridSize?.cols ?? 4 }, () => '')
  );
  return (
    <div style={{ display: 'inline-block', border: '2px solid #94a3b8', borderRadius: '4px', overflow: 'hidden', margin: '8px 0' }}>
      {rows.map((row, ri) => (
        <div key={ri} style={{ display: 'flex' }}>
          {row.map((cell, ci) => (
            <div key={ci} style={cell && cell !== '' && cell !== '□' ? gridCellFilledStyle : gridCellEmptyStyle}>
              {cell}
            </div>
          ))}
        </div>
...
const MatchingRenderer: React.FC<{
  ...
  return (
    <div style={{ display: 'flex', gap: '48px', alignItems: 'flex-start', margin: '12px 0' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', fontSize: '24px', color: '#94a3b8', paddingTop: '16px' }}>
        ⟷
      </div>
      <div style={matchColumnStyle}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
        <thead>
          <tr>
            {cols.map((c, i) => (
              <th key={i} style={{
                border: '2px solid #6366f1', padding: '10px 8px',
                backgroundColor: '#eef2ff', color: '#4338ca', fontWeight: 700,
                textAlign: 'center',
              }}>{c}</th>
            ))}
          </tr>
        ...
        <tbody>
          {Array.from({ length: 4 }).map((_, ri) => (
            <tr key={ri}>
              {cols.map((_, ci) => (
                <td key={ci} style={{
                  border: '1.5px solid #e2e8f0', padding: '14px 8px',
                  minHeight: '36px',
                }}>&nbsp;</td>
              ))}
            </tr>
...
const NumberingRenderer: React.FC<{ count: number }> = ({ count }) => (
  <div style={{ display: 'flex', gap: '8px', marginTop: '6px' }}>
    {Array.from({ length: count }).map((_, i) => (
      <span key={i} style={blankBoxStyle}>&nbsp;</span>
    ))}
  </div>
);
