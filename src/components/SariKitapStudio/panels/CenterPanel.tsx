...
export const CenterPanel: React.FC = () => {
  ...
  return (
    <div className="flex-1 h-full bg-[var(--bg-default)] flex flex-col items-center overflow-y-auto custom-scrollbar p-10">
      ...
      <div className="relative shadow-2xl shadow-black/50">
        {isGenerating ? (
          ...
          <div 
            id="sari-kitap-preview"
            className="w-[210mm] min-h-[297mm] bg-white rounded-sm overflow-hidden scale-[0.85] origin-top transform-gpu"
          >
             <SheetRenderer 
          ...
          <div className="w-[210mm] h-[297mm] bg-white/5 border-2 border-dashed border-[var(--border-color)] rounded-sm flex flex-col items-center justify-center p-12 text-center">
            <div className="w-24 h-24 rounded-[2.5rem] bg-[var(--bg-paper)]/50 flex items-center justify-center text-[var(--text-muted)] mb-8">
              <i className="fa-solid fa-file-invoice text-4xl"></i>
            </div>
            <h3 className="text-2xl font-black text-[var(--text-muted)] uppercase tracking-tighter mb-4" style={{ fontFamily: 'Lexend' }}>Önizleme Bekleniyor</h3>
            ...
            <div className="grid grid-cols-3 gap-4 mt-12 w-full max-w-md opacity-20">
              <div className="h-2 bg-[var(--text-muted)] rounded-full"></div>
              <div className="h-2 bg-[var(--text-muted)] rounded-full"></div>
              <div className="h-2 bg-[var(--text-muted)] rounded-full"></div>
              <div className="col-span-2 h-2 bg-[var(--text-muted)] rounded-full"></div>
              <div className="h-2 bg-[var(--text-muted)] rounded-full"></div>
              <div className="h-32 col-span-3 bg-[var(--bg-paper)]/50 rounded-2xl border border-[var(--border-color)]"></div>
            </div>
          </div>
        )}
      </div>
