            ...
            <div className="p-5 bg-zinc-50 dark:bg-zinc-800 rounded-[2.5rem] border border-zinc-100 dark:border-zinc-700 space-y-3">
                ...
                {[
                    ...
                    <div key={item.k} className="flex items-center justify-between">
                        ...
                        <div className={`w-8 h-4 rounded-full relative cursor-pointer transition-colors ${(options as any)[item.k] ? 'bg-indigo-600' : 'bg-zinc-300'}`} onClick={() => onChange(item.k, !(options as any)[item.k])}>
                            <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${(options as any)[item.k] ? 'left-4.5' : 'left-0.5'}`}></div>
                        </div>
                    </div>
                ))}
            </div>
