        ...
        <div className="p-4 space-y-4 animate-in fade-in duration-500">
                ...
                <div className="flex-1 bg-white dark:bg-zinc-900 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-800 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        <div className="flex justify-between items-start mb-6">
                            <div>
                                <h3 className="text-sm font-black text-zinc-900 dark:text-white tracking-tight uppercase">Genel Gelişim</h3>
                                <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest mt-0.5">Başarı Endeksi</p>
                            </div>
                        ...
                        <div className="flex items-center gap-8">
                            <div className="relative w-24 h-24 flex items-center justify-center">
                                <svg className="w-full h-full -rotate-90">
                                    <circle cx="48" cy="48" r="42" fill="none" stroke="currentColor" strokeWidth="8" className="text-zinc-100 dark:text-zinc-800" />
                                    <circle cx="48" cy="48" r="42" fill="none" stroke="currentColor" strokeWidth="8" strokeDasharray={264} strokeDashoffset={264 - (264 * getCognitiveScore()) / 100} className="text-indigo-600 transition-all duration-1000 ease-out" strokeLinecap="round" />
                                </svg>
                                <div className="absolute inset-0 flex flex-col items-center justify-center">
                                    <span className="text-2xl font-black text-zinc-900 dark:text-white">%{getCognitiveScore()}</span>
                ...
                <div className="w-full lg:w-72 bg-zinc-900 dark:bg-black rounded-3xl p-6 text-white shadow-xl relative overflow-hidden flex flex-col justify-between border border-white/5">
                    <div className="relative z-10">
                        <div className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-white/10 rounded-full border border-white/10 backdrop-blur-sm mb-4">
                            ...
                            <span className="text-[8px] font-black uppercase tracking-wider text-indigo-200">AI Öngörüsü</span>
                        </div>
                    ...
                    <div className="space-y-3 relative z-10">
                        <div className="p-2.5 bg-white/5 rounded-xl border border-white/5">
                            ...
                            <p className="text-[10px] font-medium text-indigo-50">Sabah seanslarında 'Mantık' çalışılmalı.</p>
                        </div>
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-800 shadow-sm relative overflow-hidden">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h3 className="text-xs font-black text-zinc-900 dark:text-white tracking-tight uppercase">Performans Grafiği</h3>
                        <p className="text-[8px] font-bold text-zinc-400 uppercase tracking-widest mt-0.5">Son 7 Günlük Değişim</p>
                    </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    { label: 'Çözülen Soru', value: '1,240', icon: 'fa-brain', color: 'text-indigo-500', bg: 'bg-indigo-50 dark:bg-indigo-900/20' },
                    { label: 'Toplam Süre', value: '42s', icon: 'fa-stopwatch', color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-900/20' },
                    { label: 'Başarı Oranı', value: '%94', icon: 'fa-check-double', color: 'text-amber-500', bg: 'bg-amber-50 dark:bg-amber-900/20' },
                    { label: 'Aktif Gün', value: '18', icon: 'fa-calendar-check', color: 'text-rose-500', bg: 'bg-rose-50 dark:bg-rose-900/20' }
                ].map((stat, i) => (
                    <div key={i} className="bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800 flex items-center gap-3 shadow-sm">
                        <div className={`w-8 h-8 ${stat.bg} ${stat.color} rounded-lg flex items-center justify-center text-sm`}>
                            <i className={`fa-solid ${stat.icon}`}></i>
                        </div>
                        <div>
                            <p className="text-[8px] font-black text-zinc-400 uppercase tracking-widest">{stat.label}</p>
                            <p className="text-sm font-black text-zinc-900 dark:text-white leading-none mt-1">{stat.value}</p>
                        </div>
                    </div>
                ))}
            </div>
