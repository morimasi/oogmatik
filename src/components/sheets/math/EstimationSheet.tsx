...
export const EstimationSheet = ({ data }: { data: EstimationData }) => {
    ...
    const items = resolvedData.items || rawData.items || content.items || [];

    return (
        <div className="font-lexend p-2">
            <PedagogicalHeader title={title} instruction={instruction} data={resolvedData} />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 print:gap-2 print:gap-3 print:p-3 mt-10 print:mt-3">
                {(items || []).map((item: any, idx: number) => (
                    <div key={idx} className="flex flex-col items-center gap-8 print:gap-2 print:gap-3 print:p-3 p-10 print:p-3 print:p-4 print:p-1 bg-white border-[3px] border-zinc-900 rounded-[3rem] shadow-sm break-inside-avoid group hover:border-indigo-50 transition-all">
                        <div className="w-56 h-56 border-[6px] border-zinc-100 rounded-full bg-zinc-50 relative overflow-hidden shadow-inner flex items-center justify-center">
                            <div className="absolute inset-0 flex flex-wrap content-center justify-center gap-3 p-6 print:p-2">
                        ...
                        <div className="w-full text-center">
                            <p className="text-sm font-black text-zinc-400 uppercase tracking-widest mb-6 print:mb-2">Tahminin Hangisi?</p>
                            <div className="flex justify-center gap-4 print:gap-1">
