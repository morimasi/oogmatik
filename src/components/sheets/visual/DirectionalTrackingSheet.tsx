...
export const DirectionalTrackingSheet = ({ data }: { data: DirectionalTrackingData }) => {
  ...
  return (
    <div className={`
      flex flex-col min-h-full print:min-h-0 font-sans text-black overflow-visible professional-worksheet p-6 print:p-2 
    ...
    `}>
      ...
      <div className={`grid ${isSingle ? 'grid-cols-1' : 'grid-cols-2'} gap-6 print:gap-2 flex-1 content-start`}>
                ...
                {puzzle.steps && puzzle.steps.length > 0 ? (
                  <div className="flex flex-wrap gap-1.5">
                    {puzzle.steps.map((step, i) => (
                      <div key={i} className="flex items-center gap-1.5 px-2 py-1 bg-white/5 border border-white/10 rounded-lg transition-transform hover:scale-105">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {puzzle.path.map((dir, dIdx) => (
                      <div key={dIdx} className="p-1 px-2.5 py-1 bg-white/5 border border-white/10 rounded-lg">
              ...
              <div className="flex flex-row gap-6 print:gap-2 items-center justify-center">
                <div className="relative p-3 print:p-1 bg-white rounded-2xl border-2 border-zinc-100 shadow-inner ring-4 ring-zinc-50/50">
                  ...
                  <div className="flex ml-8 mb-1">
                    {puzzle.grid[0].map((_, c) => (
                      <span key={c} className="w-9 h-5 flex items-center justify-center text-[9px] font-black text-zinc-300 uppercase tracking-widest">
                  ...
                  <div className="flex">
                    ...
                    <div className="flex flex-col mr-1">
                      {puzzle.grid.map((_, r) => (
                        <span key={r} className="w-7 h-9 flex items-center justify-center text-[9px] font-black text-zinc-300">
                          {r + 1}
                        </span>
                      ))}
                    </div>
                ...
                <div className="flex flex-col gap-3">
                    <div className="flex items-center gap-2 text-[9px] font-black text-indigo-600 uppercase tracking-widest bg-indigo-50/80 px-4 py-2 rounded-full border border-indigo-100 shadow-sm backdrop-blur-sm">
                      ...
                      BAŞLANGIÇ: <span className="text-zinc-900">{String.fromCharCode(65 + puzzle.startPos.c)}{puzzle.startPos.r + 1}</span>
                    </div>
      ...
      `}>
        <div className="flex flex-col border-r border-white/10 pr-6">
            <span className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mb-0.5">KOD ANAHTARI</span>
            <span className="text-[8px] font-bold text-zinc-500 uppercase tracking-tighter">Yönelge Protokolü v4.1</span>
        </div>
        <div className="flex gap-4 flex-wrap">
          {([
            ...
            <div key={s.sym} className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl px-4 py-2 flex items-center gap-3 hover:bg-white/10 transition-colors">
      ...
      `}>
        <div className="col-span-1 flex flex-col justify-center">
            <span className="text-[11px] font-black uppercase tracking-tight leading-none">VİZÜO-MOTOR <br />ANALİZ ANALİTİĞİ</span>
        </div>
        ...
        {[
            ...
            <div key={item.label} className="bg-white/5 border border-white/10 rounded-xl p-3 flex flex-col justify-between">
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl p-3 flex flex-col justify-between">
          ...
          <div className="flex gap-1.5">
            {[1, 2, 3, 4, 5].map(s => (
              <div key={s} className="w-4 h-4 rounded-md border border-white/10 transition-colors hover:bg-amber-400"></div>
            ))}
          </div>
        </div>
      </div>
