...
export const PatternCompletionSheet: React.FC<Props> = ({ data, settings: globalSettings }) => {
  ...
  const gridSize = rawData?.settings?.gridSize || 3;
  const patternType = rawData?.settings?.patternType || 'geometric';

  const resolvedData = rawData?.data || rawData;
  const content = resolvedData?.content || resolvedData;

  // Desteklenen tüm veri yapılarını tara (content.puzzles, puzzles, tekli matrix)
  const puzzles: any[] = content?.puzzles || resolvedData?.puzzles || rawData?.puzzles || [
    {
      id: 'pz_old',
      gridSize: gridSize,
      patternType: patternType,
      matrix: content?.matrix || resolvedData?.matrix || rawData?.matrix || [],
      options: content?.options || resolvedData?.options || rawData?.options || []
    }
  ];
