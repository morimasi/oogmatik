...
export const PatternCompletionSheet: React.FC<Props> = ({ data, settings: globalSettings }) => {
  ...
  const renderShape = (type: ShapeType, color: string) => {
    switch (type) {
      case 'circle':
        return <circle cx="50" cy="50" r="40" fill={color} stroke="black" strokeWidth="2" />;
      case 'square':
        return (
          <rect
            x="15"
      ...
      case 'star':
        return (
          <polygon
            points="50,10 61,35 88,35 66,51 75,76 50,60 25,76 34,51 12,35 39,35"
            fill={color}
            stroke="black"
            strokeWidth="2"
            strokeLinejoin="round"
      ...
      default:
        return <circle cx="50" cy="50" r="40" fill={color} />; // Fallback
    }
  };
  ...
  const renderCellContent = (cell: PatternCell) => {
    if (cell.isMissing) {
      return (
        <div className="w-full h-full  flex flex-col items-center justify-center bg-zinc-100 rounded-xl border-4 border-dashed border-zinc-300 text-5xl text-zinc-300">
    ...
    if (patternType === 'geometric' && cell.shapes) {
      return (
        <div className="w-full h-full  flex items-center justify-center p-2 relative bg-white border-2 border-slate-200 rounded-xl shadow-sm">
          <svg
            viewBox="0 0 100 100"
            className="w-full h-full  max-w-[80%] max-h-[80%] overflow-visible drop-shadow-sm"
          >
  ...
  return (
    <div
      className={`w-full h-full p-8 print:p-2 print:p-3 flex flex-col bg-white overflow-hidden text-zinc-900 print:p-0 print:border-none border border-zinc-200 ${isLandscape ? 'landscape min-h-[210mm]' : 'min-h-[297mm]'}`}
    >
      {/* BAŞLIK */}
      <div className="flex justify-between items-center border-b-4 border-sky-500 pb-4 print:pb-1 mb-8 print:mb-2">
        <div>
          <h1 className="text-4xl font-black text-sky-900 tracking-tighter uppercase">
            {data.content?.title || 'Kafayı Çalıştır!'}
          </h1>
          <p className="text-sm font-bold text-sky-600 mt-1 uppercase tracking-widest">
      ...
      <div
        className={`flex-1 flex ${isLandscape ? 'flex-row' : 'flex-col lg:flex-row'} items-center justify-center gap-12 print:gap-3 print:gap-4 print:gap-1 page-break-inside-avoid px-8 print:px-2`}
      >
        {/* MATRIX (IZGARA) ALANI */}
        ...
        <div className="w-full lg:w-1/2 flex flex-col gap-6 print:gap-2 max-w-[400px]">
          <h3 className="text-xl font-black text-slate-800 uppercase tracking-widest text-center mb-2">
            Şıklar
          </h3>
      ...
      <div className="pt-4 print:pt-1 mt-auto border-t-2 border-zinc-100 flex justify-between items-center text-[10px] font-black text-slate-300 uppercase tracking-widest">
        ...
        <span>
          Modül: Kafayı Çalıştır • IQ Matrisi: {gridSize}x{gridSize}
        </span>
      </div>
    </div>
  );
