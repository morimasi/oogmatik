          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col items-center flex-1 justify-center relative py-6 print:py-2">
                  <div className="relative p-8 print:p-6 bg-white rounded-3xl shadow-xl border-2 border-zinc-100 w-full max-w-[400px] aspect-square flex items-center justify-center">
                      ...
                      {/* Izgara (Grid) - Print Friendly Border Yapısı */}
                      <div 
                          className="grid w-full h-full border-b border-r border-zinc-900 rounded-sm overflow-hidden"
                          style={{ 
                      ...
                      >
                          {puzzle.grid.map((row: any, r: number) => row.map((cell: any, c: number) => (
                              <div 
                                key={`${r}-${c}`} 
                                className="bg-white flex items-center justify-center relative border-t border-l border-zinc-900 min-w-0 min-h-0"
                              >
                                  {cell.type === 'start' && (
                                    <div className="w-4/5 h-4/5 bg-emerald-50 rounded-full flex items-center justify-center border-2 border-emerald-500">
                                      <span className="text-[10px]">🎯</span>
                                    </div>
                                  )}
                                  {cell.type === 'target' && (
                                    <div className="w-4/5 h-4/5 bg-amber-50 rounded-full flex items-center justify-center border-2 border-amber-500">
                                      <span className="text-[10px]">🏁</span>
                                    </div>
                                  )}
                                  {cell.type === 'obstacle' && (
                                    <span className="text-[14px] opacity-40 grayscale filter">{cell.icon || '🚫'}</span>
                                  )}
                              </div>
                          )))}
                      </div>
              ...
              <div className="mt-6 bg-zinc-900 text-white p-5 rounded-3xl border border-zinc-800 shadow-2xl relative overflow-hidden">
                  ...
                  <div className="flex flex-wrap gap-2 justify-start relative z-10">
                      {puzzle.instructions.map((ins: any, iIdx: number) => (
                          <div 
                              key={iIdx} 
                              className="bg-white/10 border border-white/10 rounded-xl px-3 py-1.5 flex items-center gap-2 shadow-sm font-mono"
                          >
                              {cipherType === 'arrows' ? (
                                  <>
              ...
              <div className="mt-4 flex justify-between items-center text-[7px] font-black text-zinc-400 px-4 uppercase tracking-widest border-t border-zinc-100 pt-4">
                  ...
                  <span className="opacity-40 italic">A4 PRO PACK V2.5</span>
              </div>
            </div>
          );
        })}
      ...
      <div className="mt-6 p-6 bg-zinc-950 text-white rounded-[3rem] flex justify-between items-center relative overflow-hidden shadow-2xl border border-white/5">
          ...
          <div className="flex gap-4 pr-6">
              <div className="flex flex-col items-end">
                  <span className="text-[8px] font-black text-zinc-500 uppercase mb-1">SİSTEM</span>
                  <span className="text-[10px] font-black text-indigo-300">{(settings?.cipherType || 'OKLAR').toUpperCase()}</span>
              </div>
          </div>
      </div>
    </div>
