...
export const SyllableMasterLabSheet = ({ data }: { data: SyllableMasterLabData }) => {
  ...
  const renderItem = (item: any, idx: number) => {
    ...
    return (
      <EditableElement
        key={idx}
        className="flex flex-col gap-1 p-3 border-[2px] border-zinc-900 rounded-[1.5rem] bg-white group hover:border-indigo-500 transition-all shadow-sm break-inside-avoid relative overflow-hidden"
      >
        <div className="flex-1">
          {isSplit && (
            <div className="flex flex-col gap-2">
              <h4 className="text-lg font-black tracking-widest text-zinc-800 uppercase text-center">
                {item.word}
              </h4>
            ...
            <div className="flex items-center justify-center gap-1.5 flex-wrap py-1">
              {item.syllables.map((s: any, sIdx: number) => {
                ...
                return (
                  <div
                    key={sIdx}
                    className="px-3 py-1.5 rounded-lg border-[2.5px] flex items-center justify-center min-w-[35px]"
  ...
  return (
    <div className="flex flex-col bg-white font-lexend p-1 overflow-visible">
      <PedagogicalHeader
        title={data.title}
