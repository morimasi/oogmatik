...
export const FindLetterPairSheet = ({ data }: { data: FindLetterPairData }) => {
  ...
  return (
    <div className="flex flex-col bg-white p-2 font-lexend text-black overflow-visible">
      ...
      <div className={`grid ${gridColsClass} gap-x-12 gap-y-16 mt-8 print:mt-2 content-start`}>
        {(grids || []).map((item, idx) => (
          <div key={idx} className="flex flex-col items-center break-inside-avoid group">
            {/* HEDEF GÖSTERGESİ */}
            <EditableElement className="mb-6 print:mb-2 flex flex-col items-center">
              <span className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.3em] mb-2">
                HEDEF İKİLİ
              </span>
              <div className="px-12 print:px-3 py-4 print:py-1 bg-zinc-900 text-white rounded-[1.5rem] font-black text-4xl shadow-xl border-4 border-white ring-8 ring-zinc-50 transform group-hover:scale-110 transition-transform duration-500">
                <EditableText value={item.targetPair} tag="span" />
              </div>
            </EditableElement>

            {/* HARF TABLOSU */}
            <div className="bg-white border-[5px] border-zinc-900 p-4 print:p-1 rounded-[2.5rem] shadow-2xl overflow-hidden relative group-hover:border-indigo-500 transition-colors">
            ...
            <div className="mt-8 print:mt-2 flex gap-6 print:gap-2 w-full max-w-[220px]">
              <div className="flex-1 text-center">
                <span className="text-[9px] font-black text-zinc-300 uppercase tracking-widest">
                  BULUNAN
                </span>
                <div className="h-10 border-b-2 border-dashed border-zinc-200 flex items-center justify-center font-bold text-zinc-200 italic text-sm">
      ...
      <div className="mt-auto pt-8 print:pt-2 flex justify-between items-center px-10 print:px-3 border-t border-zinc-100 opacity-30">
