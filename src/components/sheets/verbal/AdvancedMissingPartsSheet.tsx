...
export const AdvancedMissingPartsSheet: React.FC<{ data: any }> = React.memo(({ data }) => {
    ...
    const content = resolvedData?.content || rawData?.content || (data as any);

    // Paragraflar veya items dizisini yakala
    ...
    // WordBank formatı: { words: [...] } veya [...]
    const wordBankData = content.wordBank || [];
    const wordBank: string[] = Array.isArray(wordBankData)
        ? wordBankData
        : (Array.isArray(wordBankData?.words) ? wordBankData.words : []);
