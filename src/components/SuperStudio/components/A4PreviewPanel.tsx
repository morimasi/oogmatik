            ...
            <div className="h-16 border-b border-slate-800 bg-slate-900/40 backdrop-blur-xl flex items-center justify-between px-6 z-20 shadow-lg">
                <div className="flex items-center text-slate-400 text-sm">
                    <span className="text-teal-400 mr-3 relative flex h-3 w-3">
                        {isGenerating && <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-teal-400 opacity-75"></span>}
                        <span className={`relative inline-flex rounded-full h-3 w-3 ${isGenerating ? 'bg-teal-500' : 'bg-slate-600'}`}></span>
                    </span>
                    <span className="font-semibold tracking-wide uppercase text-xs text-slate-500">A4 Canlı Önizleme</span>
                    {generatedContents.length > 0 && <span className="ml-3 bg-teal-500/10 text-teal-400 px-2.5 py-0.5 rounded-full text-[10px] font-bold border border-teal-500/20">{generatedContents.length} Sayfa</span>}
                </div>
                    ...
                    {isGenerating ? (
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center space-y-8 z-10">
                                ...
                                <div className="text-center space-y-2">
                                    <p className="text-teal-400 font-bold text-2xl tracking-tight">İçerikler Dokunuyor</p>
                                    <p className="text-slate-500 text-sm font-medium">Bilişsel Yük Filtreleniyor... Pedagojik Katmanlar Oluşturuluyor...</p>
                                </div>
                            </div>
                        </motion.div>
                        ...
                        >
                            <div className="text-center space-y-6 max-w-md">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <p className="text-slate-400 text-sm">Sol taraftaki şablon menüsünden bir veya daha fazla şablon seçerek "Premium Üretim" butonuna basın.</p>
                                </div>
                            </div>
                        </motion.div>
                    ) : (
                        generatedContents.map((content, index) => (
                            <motion.div
                                ...
                                className="w-[210mm] min-h-[297mm] bg-white shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)] relative p-16 text-slate-800 font-lexend print:shadow-none print:m-0 print:p-0 print:w-full print:min-h-0 page-break-after-always overflow-hidden"
                            >
                                {/* Decorative Corner */}
                                ...
                                {/* Header Section */}
                                <div className="flex justify-between items-start border-b-4 border-slate-900 pb-8 mb-12 relative">
                                    ...
                                    <div className="grid grid-cols-2 gap-x-8 gap-y-4 text-[13px] font-bold pt-2">
                                        <div className="flex items-center text-slate-400 uppercase tracking-tighter">İSİM: <span className="ml-2 w-32 border-b-2 border-slate-200 h-5"></span></div>
                                        <div className="flex items-center text-slate-400 uppercase tracking-tighter">SINIF: <span className="ml-2 w-16 border-b-2 border-slate-200 h-5"></span></div>
                                        <div className="flex items-center text-slate-400 uppercase tracking-tighter">TARİH: <span className="ml-2 w-32 border-b-2 border-slate-200 h-5"></span></div>
                                        <div className="flex items-center text-slate-400 uppercase tracking-tighter">NO: <span className="ml-2 w-16 border-b-2 border-slate-200 h-5"></span></div>
                                    </div>
                                </div>

                                {/* Content Body */}
                                <div className="min-h-[200mm] relative">
                                    <MarkdownRenderer 
                                    ...
                                    <div className="bg-slate-50/80 border-2 border-dashed border-slate-200 p-6 rounded-2xl text-[13px] leading-relaxed text-slate-600 print:hidden transition-colors group-hover:bg-slate-50 group-hover:border-teal-200/50">
                                        <div className="flex items-start gap-4">
                                            ...
                                            <div className="space-y-1">
                                                <span className="font-bold text-slate-800 block">Stratejik Uygulama Notu:</span>
                                                {content.pages[0].pedagogicalNote}
                                            </div>
                                        </div>
                                    </div>
                                    
                                    <div className="flex justify-between items-center mt-8 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
                                        <span>Bursa Disleksi & Özel Öğrenme Güçlüğü Akademisi</span>
                                        <span>Sayfa 0{index + 1}</span>
                                    </div>
                                </div>
