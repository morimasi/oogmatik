...
import { LetterConnectData } from '../types';

interface LetterConnectSheetProps {
  data: LetterConnectData;
}
...
export const LetterConnectSheet: React.FC<LetterConnectSheetProps> = ({ data }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6 pb-3 border-b-2 relative" style={{ borderColor: `${iconColor}15` }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            >
              {isGirlMode ? 'Prenses Harf Bağlama' : 'Harf Bağlama'}
            </h1>
        ...
        <div className="flex flex-col gap-4 w-1/3">
          {items.map((item) => (
            <div key={`left-${item.id}`} className="flex items-center justify-between group">
              <div
                className="w-14 h-14 rounded-xl bg-white flex items-center justify-center shadow-sm transition-colors"
        ...
        <div className="flex flex-col gap-4 w-1/3">
          {shuffle(items).map((item) => (
            <div key={`right-${item.id}`} className="flex items-center justify-between flex-row-reverse group">
              <div
                className="w-14 h-14 rounded-xl flex items-center justify-center shadow-sm transition-colors cursor-pointer"
