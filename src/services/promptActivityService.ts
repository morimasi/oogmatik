...
function getQuickDefaults(_grade: number): Partial<PromptGenerationRequest> {
  return {
...
const PROMPT_ANALYSIS_SCHEMA = {
  ...
  properties: {
    ...
    suggestedQuestionTypes: {
      ...
      items: { type: 'STRING' as const },
      description:
        'Önerilen soru tipleri (multiple_choice, fill_in_the_blank, matching, true_false, open_ended, ordering)',
    },
    suggestedDifficulty: {
      type: 'STRING' as const,
...
6. Süre tahmini: dakika cinsinden (5-45 arası).
    ...
    return {
      ...
      suggestedQuestionTypes: ((data['suggestedQuestionTypes'] as string[]) ||
        []) as QuestionType[],
