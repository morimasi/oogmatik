...
import { SUPER_STUDIO_REGISTRY, getTemplateById } from '../../components/SuperStudio/templates/registry';
...
const buildSchemaForTemplate = (_templateId: string): any => {
    return {
        ...
        properties: {
            ...
            content: { type: 'STRING', description: 'Tüm yönerge, metin, sorular ve SVG\'leri içeren zengin Markdown içeriği' },
...
export const generateSuperStudioContent = async (
    ...
    try {
        ...
        const successes = settled
            .filter((r): r is PromiseFulfilledResult<{success: boolean, templateId: string, data: GeneratedContentPayload}> =>
                r.status === 'fulfilled'
            )
            .map(r => r.value.data);
