...
export const generateOfflineWordMemory = async (options: GeneratorOptions): Promise<WordMemoryData[]> => {
    ...
    for (let i = 0; i < (worksheetCount || 1); i++) {
        ...
        let pool = getWordsForDifficulty(difficulty || 'orta', topic || 'Rastgele');
        if (pool.length < count * 2) pool = [...pool, ...getWordsForDifficulty(difficulty || 'orta', 'Rastgele')];
...
export const generateOfflineTargetSearch = async (options: GeneratorOptions): Promise<TargetSearchData[]> => {
    const { gridSize, worksheetCount, targetChar, distractorChar } = options;
