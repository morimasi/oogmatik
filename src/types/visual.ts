...
export interface VisualOddOneOutData extends BaseActivityData {
    settings?: {
        ...
        itemType: 'svg' | 'text' | 'image' | 'character' | 'emoji' | 'mixed' | 'abstract';
...
export interface GridDrawingData extends BaseActivityData {
    settings?: {
        difficulty: 'beginner' | 'intermediate' | 'expert' | 'clinical';
        layout: 'side_by_side' | 'stacked' | 'grid_2x2';
...
export interface VisualInterpretationQuestion {
    id: string;
    type: 'true_false' | 'multiple_choice' | 'open_ended' | '5n1k';
    text: string;
    options?: string[];
    answer?: string;
    isTrue?: boolean;
...
export interface VisualInterpretationData extends BaseActivityData {
    settings?: {
    ...
    questions: VisualInterpretationQuestion[];
    observationNotesArea?: boolean;
}
