...
export const OCRScanner: React.FC<OCRScannerProps> = ({ onBack }) => {
        ...
        if (file) {
            try {
                ...
                if (file.type.startsWith('image/')) {
                    ...
                    alert("Lütfen geçerli bir resim (JPG, PNG) dosyası seçin.");
                    return;
                }
    ...
    const startAnalysis = async (imgData: string) => {
        ...
        try {
            ...
            alert("Görsel analiz edilemedi. Lütfen daha net bir fotoğraf yükleyin.");
            setStep('upload');
            setImage(null);
        }
    };
            ...
            <div className="h-16 bg-[#18181b] border-b border-zinc-800 flex justify-between items-center px-6 shrink-0 z-10">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        {step === 'studio' && <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Algoritma Çözümleyici</p>}
                    </div>
                </div>
                
                {step === 'preview' && (
                    <div className="flex gap-2">
                        <button onClick={() => printService.generatePdf('.worksheet-page', config.title, { action: 'print' })} className="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-white rounded-lg font-bold shadow-sm transition-all"><i className="fa-solid fa-print mr-2"></i> Yazdır</button>
                        <button onClick={handleSaveToArchive} disabled={!user || isSaving} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-bold shadow-sm transition-all disabled:opacity-50">
                            {isSaving ? <i className="fa-solid fa-circle-notch fa-spin"></i> : <i className="fa-solid fa-save mr-2"></i>} Kaydet
                        </button>
                    </div>
                )}
            </div>
            ...
            <div className="flex-1 overflow-hidden relative flex flex-col">
                ...
                {step === 'upload' && (
                    <div className="h-full flex flex-col items-center justify-center p-8 bg-[#121212] bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]">
                        <div className="mb-10 flex gap-4 bg-zinc-900 p-2 rounded-2xl border border-zinc-800 shadow-xl">
                            ...
                            >
                                <i className="fa-solid fa-wand-sparkles"></i> Materyal Dönüştürücü
                            </button>
                            <button 
                        ...
                        >
                            ...
                            <h3 className="text-2xl font-bold text-white relative z-10">Görsel veya PDF Yükle</h3>
                            <p className="text-zinc-500 mt-2 text-center px-12 text-sm relative z-10">
                                {scanMode === 'CONVERTER' ? 'Yapay zeka kağıt düzenini analiz eder ve yeni sorularla klonlar.' : 'Yapay zeka görseldeki mantığı çözerek bir akış şeması oluşturur.'}
                            </p>
                        </div>
                ...
                {step === 'processing' && (
                    <div className="h-full flex flex-col items-center justify-center bg-[#121212]">
                        ...
                        <h3 className="text-xl font-bold text-white">Gemini 3.0 Flash Mimarisi Analiz Ediyor...</h3>
                        <p className="text-zinc-500 mt-2 text-sm">Görselin mantığı ve yapısı çözümleniyor.</p>
                    </div>
                )}
                ...
                {step === 'studio' && analysisResult && (
                    <div className="h-full flex flex-col lg:flex-row bg-[#121212]">
                        <div className="flex-1 p-6 flex flex-col gap-6 overflow-y-auto custom-scrollbar">
                            ...
                            <div className="flex-1 bg-[#1e1e1e] rounded-2xl border border-zinc-800 shadow-inner flex flex-col overflow-hidden min-h-[400px]">
                                <div className="bg-[#252526] border-b border-zinc-800 p-2 flex items-center justify-between">
                                    <span className="text-[10px] font-bold text-zinc-400 px-3 uppercase tracking-wider">Üretim Algoritması</span>
                                    <div className="flex gap-2">
                                        {STUDIO_TOOLS.map(tool => (
                                            <button 
                                                key={tool.id}
                                                onClick={() => insertSnippet(tool.snippet)}
                                                className="px-3 py-1.5 bg-[#333] border border-zinc-700 rounded-md text-[10px] font-bold text-zinc-300 hover:bg-zinc-700 hover:text-white transition-all"
                                            >
                                                <i className={`fa-solid ${tool.icon} mr-1.5`}></i>{tool.label}
                                            </button>
                                        ))}
                                    </div>
                                </div>
                                <textarea 
                                    ...
                                    className="flex-1 p-6 bg-[#1e1e1e] text-emerald-400 font-mono text-sm leading-relaxed resize-none outline-none"
                                    placeholder="// Analiz edilen mantık buraya gelecek..."
                                    spellCheck={false}
                                />
                            </div>
                        ...
                        <div className="hidden lg:flex w-96 bg-[#18181b] border-l border-zinc-800 items-center justify-center p-6 relative shrink-0">
                             <div className="absolute top-4 left-4 bg-black/60 text-white px-3 py-1 rounded-full text-[10px] font-bold backdrop-blur border border-white/10 uppercase tracking-widest">Orijinal Görsel</div>
                ...
                {step === 'generating' && (
                    <div className="h-full flex flex-col items-center justify-center bg-[#121212]">
                        ...
                        <h3 className="text-xl font-bold text-white">Yeni Materyal İnşa Ediliyor</h3>
                        <p className="text-zinc-500 mt-2 text-sm">Gemini 3.0 Flash Preview mantığı işliyor...</p>
                    </div>
                )}
