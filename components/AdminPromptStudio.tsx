...
const CodeEditor = ({ value, onChange, readOnly = false }: { value: string, onChange?: (v: string) => void, readOnly?: boolean }) => {
    ...
    const highlight = (code: string) => {
        return code
            ...
            .replace(/(\{\{)(.*?)(\}\})/g, '<span class="text-amber-500 font-bold bg-amber-500/10 px-1 rounded-sm">$1$2$3</span>')
            .replace(/\[(.*?)\]/g, '<span class="text-emerald-500 font-black tracking-widest uppercase text-[10px] bg-emerald-500/10 px-1">[$1]</span>')
            .replace(/(GÖREV:|ROL:|KURALLAR:|ÇIKTI:|PEDAGOJİ:|HEDEF KİTLE:)/gi, '<span class="text-rose-500 font-black border-b border-rose-500/30">$1</span>')
            .replace(/"(.*?)"/g, '<span class="text-sky-400">"$1"</span>');
    };
    ...
    return (
        <div className="relative w-full h-full bg-[#0a0a0a] rounded-xl border border-zinc-800/50 overflow-hidden font-mono text-xs shadow-inner group">
            <div className="absolute left-0 top-0 bottom-0 w-10 bg-[#050505] border-r border-zinc-800/50 flex flex-col items-center pt-4 text-zinc-700 select-none text-[9px]">
                {value.split('\n').map((_, i) => <div key={i} className="h-[22px] leading-[22px]">{i + 1}</div>)}
            </div>
            <pre ref={preRef} className="absolute inset-0 left-10 p-4 m-0 pointer-events-none whitespace-pre-wrap break-words leading-[22px] overflow-hidden">
                <code dangerouslySetInnerHTML={{ __html: highlight(value) }} />
            </pre>
            <textarea
                ref={textRef}
...
=======
    ...
    const [isSaving, setIsSaving] = (React as any).useState(false);
    const [testVars, setTestVars] = (React as any).useState({} as Record<string, string>);
    const [activeTab, setActiveTab] = (React as any).useState('editor' as 'editor' | 'history' | 'simulation');

    // A/B Testing States
    const [simA, setSimA] = (React as any).useState({ result: '', loading: false, temp: 0.1 } as { result: string, loading: boolean, temp: number });
    const [simB, setSimB] = (React as any).useState({ result: '', loading: false, temp: 0.8 } as { result: string, loading: boolean, temp: number });

    (React as any).useEffect(() => { loadData(); }, []);
    ...
    const linter = (React as any).useMemo(() => {
        ...
        const exactTokens = Math.floor(full.length / 4.2);
        let score = 100 - (warnings.filter((w: any) => w.type === 'error').length * 25) - (warnings.filter((w: any) => w.type === 'warning').length * 15);
        if (exactTokens > 1500) score -= 15; // Too long for consistent results 
    ...
    const handleSelect = async (act: any) => {
        ...
        if (!p) {
            p = {
                id, name: `${act.title} Mimarisi`, description: '', category: 'custom',
    ...
    const handleSave = async () => {
        if (!selected) return;
        const note = prompt("Mimari Evrim Kaydı (Değişiklik Logu):", "Derinlemesine kognitif optimizasyon");
        if (!note) return;
        setIsSaving(true);
        try {
            const saved = await adminService.savePrompt({ ...selected, variables }, note);
    ...
    const runSimulation = async (variant: 'A' | 'B') => {
        ...
        variant === 'A' ? setSimA((p: any) => ({ ...p, loading: true, result: '' })) : setSimB((p: any) => ({ ...p, loading: true, result: '' }));
        const t = variant === 'A' ? simA.temp : simB.temp;

        try {
            // Modify selected config temporarily just to simulate
            const tempSelected = { ...selected, modelConfig: { ...selected.modelConfig, temperature: t } };
            // Simulate variable injection inside the service or here
            const mockResult = await adminService.testPrompt(tempSelected, testVars);
            const strRes = JSON.stringify(mockResult, null, 2);
            variant === 'A' ? setSimA((p: any) => ({ ...p, result: strRes })) : setSimB((p: any) => ({ ...p, result: strRes }));
        } catch (e: any) {
            variant === 'A' ? setSimA((p: any) => ({ ...p, result: `HATA: ${e.message}` })) : setSimB((p: any) => ({ ...p, result: `HATA: ${e.message}` }));
        } finally {
            variant === 'A' ? setSimA((p: any) => ({ ...p, loading: false })) : setSimB((p: any) => ({ ...p, loading: false }));
        }
    };
                            ...
                            <div className="space-y-0.5">
                                {ACTIVITIES.filter(a => cat.activities.includes(a.id)).map(act => (
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-2 rounded-xl text-xs font-medium transition-all flex items-center gap-3 relative overflow-hidden group ${selected?.id === `prompt_${act.id.toLowerCase()}` ? 'bg-zinc-800/80 text-white shadow-inner' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/50'}`}
                                    >
                                        {selected?.id === `prompt_${act.id.toLowerCase()}` && <div className="absolute left-0 top-0 bottom-0 w-1 bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]"></div>}
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-[#050505] relative">
                {selected ? (
                    <>
                        <div className="h-16 border-b border-zinc-900/80 flex items-center justify-between px-8 bg-[#080808] shrink-0 z-10">
                            <div className="flex flex-col">
                                ...
                                <p className="text-[10px] text-zinc-600 font-mono mt-0.5" title={linter.warnings.map((w: any) => w.text).join('\n')}>
                                    Maliyet: ~{linter.tokens} tkns • Skor: <span className={linter.score >= 90 ? 'text-emerald-500' : linter.score >= 50 ? 'text-amber-500' : 'text-red-500'}>{linter.score}/100</span>
                                </p>
                            </div>
                            <div className="flex bg-zinc-900/50 p-1.5 rounded-xl border border-zinc-800/50">
                                {[
                        ...
                        <div className="flex-1 p-6 relative flex flex-col min-h-0 min-w-0">
                                ...
                                <div className="flex-1 flex gap-6 min-h-0">
                                    ...
                                    <div className="w-80 flex flex-col gap-5 shrink-0">
                                        <div className="h-1/3 bg-[#0a0a0a] rounded-2xl border border-zinc-800/50 p-5 flex flex-col relative overflow-hidden">
                                            ...
                                            <textarea
                                                value={selected.systemInstruction}
                                                onChange={(e: any) => setSelected({ ...selected, systemInstruction: e.target.value })}
                                                className="w-full flex-1 bg-transparent text-[11px] font-mono leading-relaxed text-zinc-400 outline-none resize-none custom-scrollbar z-10"
                                                placeholder="Modelin temel rolü ve düşünme biçimi..."
                                            />
                                        </div>
                            ...
                            {activeTab === 'history' && (
                                <div className="h-full space-y-4 overflow-y-auto custom-scrollbar pr-2 max-w-3xl mx-auto w-full">
                                    {(selected.history?.length || 0) === 0 && <div className="text-center p-32 text-zinc-600 flex flex-col items-center gap-4"><i className="fa-solid fa-clock-rotate-left text-5xl opacity-20"></i><p className="text-[10px] font-black uppercase tracking-widest">Kayıtsız Organizma.</p></div>}
                                    {[...(selected.history || [])].reverse().map((h, i) => (
                                        <div key={i} className="p-6 bg-[#0a0a0a] border border-zinc-800/50 rounded-2xl flex justify-between items-center group hover:border-amber-500/50 transition-all shadow-lg relative overflow-hidden">
                                            ...
                                            <button
                                                onClick={() => { if (confirm('Sistemin nöral yolları eski dna dizişine döndürülecek. Onaylıyor musunuz?')) setSelected({ ...selected, template: h.template, systemInstruction: h.systemInstruction || selected.systemInstruction }); }}
                                                className="px-6 py-2.5 bg-zinc-800 hover:bg-amber-500 hover:text-black text-white text-[10px] font-black rounded-xl opacity-0 group-hover:opacity-100 transition-all transform active:scale-95 shadow-[0_0_20px_rgba(245,158,11,0.2)]"
                                            >KLONLA & UYGULA</button>
                                        </div>
                                    ))}
                                </div>
                            )}
                        ...
                        <div className="h-20 border-t border-zinc-900/80 bg-[#080808] flex items-center px-8 justify-between shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] z-20">
                            <div className="flex gap-6">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-[10px] text-zinc-400 font-mono tracking-wider">{selected.modelConfig?.modelName || 'gemini-3-flash-preview'}</span>
                                </div>
                            </div>
                            <button
                                onClick={handleSave}
                                ...
                                className={`px-12 py-3.5 text-black font-black rounded-2xl text-[11px] uppercase tracking-widest transition-all transform active:scale-95 disabled:opacity-30 disabled:pointer-events-none flex items-center gap-3 ${linter.score < 50 ? 'bg-zinc-800 text-zinc-600' : 'bg-amber-500 hover:bg-amber-400 shadow-[0_0_30px_rgba(245,158,11,0.4)] hover:shadow-[0_0_40px_rgba(245,158,11,0.6)]'}`}
                            >
                                {isSaving ? <><i className="fa-solid fa-dna fa-spin"></i> HÜCRESEL YAZIM</> : <><i className="fa-solid fa-bolt"></i> MİMARİYİ ZİHNİNE KAZI</>}
                            </button>
                        </div>
                    </>
                ) : (
                    <div className="flex-1 flex flex-col items-center justify-center text-zinc-800 p-20 text-center select-none bg-gradient-to-b from-[#050505] to-[#020202]">
                        ...
                        <p className="max-w-md text-[11px] text-zinc-600 leading-relaxed font-medium uppercase tracking-widest">Özel eğitim etkinliklerinin yapay zeka tarafından nasıl düşünüleceğini yapılandırmak için sol havuzdan bir aktivite nöronu seçin. Negatif dil kullanmaktan kaçının.</p>
                    </div>
                )}
            </div>
                ...
                <div className="w-72 border-l border-zinc-900 bg-[#080808] flex flex-col shrink-0 z-10">
                    <div className="p-5 border-b border-zinc-900/50 flex flex-col gap-1">
                        <div className="flex justify-between items-center">
                            ...
                            <button onClick={() => setSnippets([...snippets, { id: `new_${Date.now()}`, label: 'YENİ BLOK', value: '...' }])} className="text-zinc-600 hover:text-emerald-500 transition-colors"><i className="fa-solid fa-plus-circle"></i></button>
                        </div>
                        <p className="text-[9px] text-zinc-600 font-medium">Sık kullanılan pedagojik parametreler</p>
                    </div>
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-3 bg-[#050505]">
                        {snippets.length === 0 && <div className="text-center p-10 text-zinc-700 text-[9px] uppercase tracking-widest font-black">Havuz Boş</div>}
                        {snippets.map((s: PromptSnippet) => (
                            <div key={s.id} className="group p-4 bg-[#0a0a0a] border border-zinc-800/50 rounded-2xl relative hover:border-emerald-500/40 transition-all cursor-pointer shadow-lg overflow-hidden" onClick={() => selected && setSelected({ ...selected, template: selected.template + "\n\n" + s.value })}>
                                <div className="absolute top-0 right-0 w-16 h-16 bg-emerald-500/5 blur-2xl pointer-events-none"></div>
                                ...
                                <button onClick={(e: any) => { e.stopPropagation(); const val = prompt("Yönerge İçeriği:", s.value); if (val) adminService.saveSnippet({ ...s, value: val }); }} className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 text-zinc-600 hover:text-emerald-500 transition-colors z-20"><i className="fa-solid fa-pen text-[9px]"></i></button>
                                <div className="absolute inset-x-0 bottom-0 h-0.5 bg-emerald-500/80 scale-x-0 group-hover:scale-x-100 transition-transform origin-left"></div>
                            </div>
                        ))}
                    </div>
