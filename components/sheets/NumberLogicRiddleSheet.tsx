...
export const NumberLogicRiddleSheet: React.FC<{ data: NumberLogicRiddleData }> = ({ data }) => {
    return (
        <div className="h-full flex flex-col text-black font-lexend">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 print:grid-cols-2 gap-x-8 gap-y-10 mt-4 flex-1">
                {(data.puzzles || []).map((puzzle, pIdx) => (
                    <EditableElement key={pIdx} className="flex flex-col border-4 border-zinc-200 rounded-[2rem] p-6 bg-white shadow-sm break-inside-avoid relative group hover:border-indigo-400 transition-all duration-300 flex-1">
                        <div className="absolute -top-4 -left-4 w-12 h-12 bg-zinc-900 text-white rounded-2xl flex items-center justify-center font-black shadow-lg text-lg ring-4 ring-white">
                            {pIdx + 1}
                        </div>
                        
                        {/* Riddle Text - Adjusted for better space usage */}
                        <div className="mb-4 bg-zinc-50 p-4 rounded-2xl border border-zinc-100 min-h-[90px] flex items-center shadow-inner">
                            <p className="text-sm font-bold leading-relaxed text-zinc-800">
                                <EditableText value={puzzle.riddle} tag="span" />
                            </p>
                        </div>

                        {/* Number Logic Grid - Optimized for A4 */}
                        <div className="grid grid-cols-5 gap-2 mb-6">
                            {puzzle.boxes.map((box, bIdx) => (
                                <div key={bIdx} className="flex flex-col items-center bg-white border-2 border-zinc-100 rounded-xl p-2 shadow-sm min-h-[75px] justify-center hover:border-indigo-200 transition-colors">
                                    <div className="text-[7px] font-black text-zinc-300 uppercase mb-1 tracking-tighter">GRUP {bIdx+1}</div>
                                    <div className="flex flex-wrap justify-center gap-1">
                                        {box.map((num, nIdx) => (
                                            <span key={nIdx} className="text-xs font-black font-mono bg-zinc-50 px-1 rounded text-zinc-700">{num}</span>
                                        ))}
                                    </div>
                                </div>
                            ))}
                        ...
                        {/* Options Strip */}
                        <div className="mt-auto pt-4 border-t-2 border-zinc-50">
                            <div className="flex justify-between items-center gap-1">
                                {puzzle.options.map((opt, oIdx) => (
                                    <div key={oIdx} className="flex flex-col items-center gap-1 cursor-pointer group/opt">
                                        <div className="text-[9px] font-black text-zinc-400 uppercase tracking-widest group-hover/opt:text-indigo-500 transition-colors">{String.fromCharCode(97 + oIdx)})</div>
                                        <div className="w-10 h-10 rounded-xl border-2 border-zinc-100 flex items-center justify-center font-black text-sm shadow-sm group-hover/opt:bg-indigo-50 group-hover/opt:border-indigo-400 transition-all active:scale-95 bg-white">
            ...
            {/* Sum Challenge Footer - Adjusted to stick to bottom and fill width */}
            <div className="mt-8 mb-2 break-inside-avoid shrink-0">
                <EditableElement className="bg-gradient-to-br from-zinc-900 to-zinc-800 text-white p-6 rounded-[2.5rem] shadow-xl relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6">
                    ...
                    <div className="relative z-10 text-center md:text-left flex-1">
                        ...
                        <p className="text-xs text-zinc-400 font-medium max-w-md leading-tight">
