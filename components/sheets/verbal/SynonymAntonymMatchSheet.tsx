...
export const SynonymAntonymMatchSheet = ({ data }: { data: SynonymAntonymMatchData }) => {
  ...
  return (
    <div className="flex flex-col bg-white font-lexend text-black p-2 overflow-visible">
      <PedagogicalHeader
        title={data.title}
            ...
            {shuffledTargets.map((pair, idx) => (
              ...
              >
                ...
                <span className="font-bold text-xl uppercase ml-6 text-zinc-500 group-hover:text-zinc-900">
                  <EditableText value={pair.target} tag="span" />
                </span>
              </div>
            ))}
          </div>
              ...
              <p className="text-xl leading-relaxed italic text-zinc-800">
                {sent.text.split('_______').map((part, i, arr) => (
                  <React.Fragment key={i}>
                    {part}
                    {i < arr.length - 1 && (
                      <span className="inline-block min-w-[140px] border-b-2 border-dashed border-indigo-400 mx-2 text-transparent">
                        ..........
                      </span>
                    )}
                  </React.Fragment>
