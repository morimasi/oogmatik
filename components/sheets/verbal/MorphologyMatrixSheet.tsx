...
export const MorphologyMatrixSheet = ({ data }: { data: MorphologyMatrixData }) => {
  ...
  return (
    <div className="flex flex-col bg-white font-sans text-black overflow-visible professional-worksheet">
      ...
      <div
        className={`grid ${isGrid ? 'grid-cols-2' : 'grid-cols-1'} gap-8 print:gap-3 print:p-3 mt-10 content-start pb-20`}
      >
        {(data?.items || []).map((item, idx) => (
          ...
          >
            ...
            <div className="flex items-center gap-8 print:gap-3 print:p-3">
              ...
              <div className="flex-1 flex flex-wrap gap-3">
                {item.suffixes.map((s, sIdx) => (
                  <div
                    key={sIdx}
                    className="bg-white border-2 border-zinc-200 rounded-2xl px-5 py-2 text-center font-black text-sm text-zinc-900 hover:border-indigo-500 hover:text-indigo-600 hover:shadow-md transition-all cursor-pointer transform hover:-translate-y-1"
                  >
