...
export const ReadingFlowSheet = ({ data }: { data: ReadingFlowData }) => (
  <div className="flex flex-col font-lexend p-2 relative overflow-visible">
    <ReadingRuler />
    <PedagogicalHeader
      title={data.title}
          ...
          {(p.sentences || []).map((s, sIdx) => (
            ...
            >
              {(s.syllables || []).map((syl, sylIdx) => (
                <span
                  key={sylIdx}
                  className="hover:bg-yellow-100 px-1 rounded transition-colors cursor-help border-b-2 border-zinc-200"
                >
