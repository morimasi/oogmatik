...
export const BrainTeasersSheet: React.FC<BrainTeasersSheetProps> = ({ data, settings }) => {
    ...
    return (
        <div className="w-full h-full flex flex-col gap-8 p-8" style={{ fontFamily: settings.fontFamily }}>
            {/* Header */}
            {settings.showTitle && (
                <div className="text-center mb-8 pb-4 border-b-2 border-dashed border-zinc-200">
                    ...
                    <h1 className="text-4xl font-black text-zinc-800 uppercase tracking-tight" style={{ color: settings.borderColor }}>
                        {activity.title}
                    </h1>
            ...
            <div className="grid grid-cols-1 gap-8">
                {puzzles.map((puzzle: any, idx: number) => (
                    <div key={idx} className="bg-white rounded-3xl border-2 border-zinc-100 shadow-xl overflow-hidden group hover:border-amber-200 transition-colors">
                        <div className="p-8">
                            <div className="flex items-start gap-6">
                                ...
                                <div className="flex-1">
                                    <h3 className="text-2xl font-bold text-zinc-800 leading-relaxed mb-6" style={{ lineHeight: settings.lineHeight }}>
                                        {puzzle.q}
                                    </h3>
                                    
                                    {/* Visual Content if available */}
                                    {puzzle.visual && (
                                        <div className="my-6 p-4 bg-zinc-50 rounded-xl border border-zinc-200 flex justify-center">
                                    ...
                                    {/* Answer Area */}
                                    <div className="mt-8 p-6 bg-zinc-50 rounded-2xl border border-zinc-100 flex justify-between items-center group/answer">
                                        <span className="text-xs font-black text-zinc-400 uppercase tracking-widest">CEVABINIZ:</span>
                                        <div className="flex-1 h-8 border-b-2 border-zinc-300 border-dashed mx-4"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div className="mt-auto pt-8 flex justify-center">
                <div className="flex items-center gap-2 px-6 py-3 bg-zinc-900 text-white rounded-full text-xs font-black uppercase tracking-widest shadow-lg">
                    <i className="fa-solid fa-brain text-amber-400"></i>
                    Beyin Gücü Skoru: 
                    <div className="flex gap-1 ml-2">
                        {[1,2,3,4,5].map(i => (
                            <div key={i} className="w-4 h-4 rounded-full border-2 border-zinc-700 bg-zinc-800"></div>
                        ))}
                    </div>
                </div>
            </div>
