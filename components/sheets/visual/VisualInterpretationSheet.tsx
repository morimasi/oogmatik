...
export const VisualInterpretationSheet: React.FC<VisualInterpretationSheetProps> = ({
  ...
  const questionsBlock = blocks.find((b) => (b.type as string) === 'questions');
  ...
  const imageUrl =
    ...
    `https://placehold.co/800x400/png?text=${encodeURIComponent(imagePrompt || 'Görsel Yükleniyor...')}`;
    ...
    >
      ...
      {settings.showTitle && (
        <div className="text-center mb-4">
          ...
          {settings.showInstruction && (
            <p className="text-lg text-zinc-500 font-medium mt-2 italic">{activity.instruction}</p>
          )}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
        {questions.map((q: any, idx: number) => (
          <div
            key={idx}
            className="bg-white/50 border border-zinc-200 rounded-2xl p-6 relative break-inside-avoid"
          >
            ...
            <h3
              className="text-lg font-bold text-zinc-800 mb-4 pl-4"
              ...
              <div className="space-y-3 pl-4">
                {q.options?.map((opt: string, i: number) => (
                  <label key={i} className="flex items-center gap-3 cursor-pointer group">
                    ...
                    <span className="text-base font-medium text-zinc-600 group-hover:text-zinc-900">
                      {opt}
                    </span>
                  </label>
                ))}
