...
const SimpleSheet = ({ data, children }: { data: any, children?: any }) => (
    <div>
                ...
                <div key={pIdx} className="mb-6 last:mb-0">
                    {(p.sentences || []).map((s: any, sIdx: number) => (
                        <p key={sIdx} className="text-xl leading-loose font-dyslexic text-zinc-800 mb-2">
                            {(s.syllables || []).map((syl: any, sylIdx: number) => (
            ...
            {(data.trains || []).map((train: any, i: number) => (
                <div key={i} className="flex flex-col gap-2">
                    <div className="flex items-center gap-2 overflow-x-auto pb-4">
                        <div className="w-16 h-16 bg-zinc-900 rounded-l-2xl flex items-center justify-center text-white text-2xl shrink-0 shadow-lg relative">
                            <i className="fa-solid fa-train"></i>
                            {train.isPseudo && (
                                <span className="absolute -top-2 -right-2 bg-rose-500 text-[8px] px-1 rounded-full text-white font-black uppercase">Pseudo</span>
                            )}
                        </div>
                        {train.syllables.map((syl: string, j: number) => {
                            const isMissing = j === train.missingSyllableIndex;
                            return (
                                <div key={j} className="flex items-center gap-1 shrink-0">
                                    <div className={`w-20 h-16 border-2 rounded-xl flex items-center justify-center font-black text-lg shadow-sm transition-all ${isMissing ? 'border-dashed border-indigo-400 bg-indigo-50/50' : 'border-zinc-800 bg-white'}`}>
                                        {isMissing ? (
            ...
            <svg viewBox={`0 0 ${data.width} ${data.height}`} className="w-full h-full overflow-visible">
                {data.paths.map((path: any) => (
                    <g key={path.id}>
                        ...
                        />
                        {/* Start Node */}
                        <circle cx="50" cy={path.yStart} r="15" fill="white" stroke={path.color} strokeWidth="2.5" />
                        <text x="50" y={path.yStart + 4} textAnchor="middle" fontSize="12" fontWeight="900" fill={path.color}>{path.startLabel || path.id}</text>
                    </g>
                ))}

                {/* Olası Tüm Çıkış Noktaları (Sağ Taraf) */}
                {Array.from({ length: data.paths.length }).map((_, i: number) => {
                    const lineCount = data.paths.length;
                    const yPos = 50 + (i * 350) / lineCount;
                    const endChar = (i + 1).toString();

                    return (
                        <g key={`end-${i}`}>
                            <circle cx="750" cy={yPos} r="18" fill="white" stroke="#e5e7eb" strokeWidth="2" strokeDasharray="4 2" />
                            <text x="750" y={yPos + 4} textAnchor="middle" fontSize="12" fontWeight="bold" fill="#94a3b8">{endChar}</text>
                        </g>
                    );
                })}
            </svg>
        ...
        {data.subType === 'letter-cancellation' && data.grid && (
            <div className="mt-4">
                <div className="mb-4 flex gap-4 justify-center">
                    ...
                    {data.targetChars?.map((c: string, i: number) => <span key={i} className="px-3 py-1 bg-rose-50 text-rose-600 rounded-full font-black">{c}</span>)}
                </div>
