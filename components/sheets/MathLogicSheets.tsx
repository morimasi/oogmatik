...
export const MathPuzzleSheet = ({ data }: { data: MathPuzzleData }) => (
    <div className="flex flex-col h-full print:h-0 font-lexend text-black bg-white">
            ...
            {(data?.puzzles || []).map((puzzle, index) => (
                <EditableElement key={index} className="flex flex-col border-[3px] border-zinc-900 rounded-[2.5rem] bg-white shadow-[10px_10px_0px_0px_rgba(0,0,0,0.05)] overflow-hidden group hover:shadow-xl transition-all duration-500 break-inside-avoid">
                    ...
                    <div className="p-8 print:p-2 print:p-3 flex-1 flex flex-col gap-4 print:gap-1">
                        ...
                        <div className="flex justify-center gap-6 print:gap-2 mb-4 print:mb-1">
                        ...
                        {/* Final Challenge Box */}
                        <div className="mt-8 print:mt-2 p-6 print:p-2 bg-indigo-50 rounded-[2rem] border-2 border-indigo-100 flex flex-col items-center gap-4 print:gap-1 relative overflow-hidden group/final">
        ...
        <div className="mt-auto pt-8 print:pt-2 flex justify-between items-center px-10 print:px-3 opacity-30 border-t border-zinc-100">
        ...
        <div className="space-y-10">
            {(data?.problems || []).map((problem, index) => (
                <EditableElement key={index} className="border-b-2 border-black pb-8 print:pb-2 break-inside-avoid item-card flex flex-col gap-2">
                    <div className="flex gap-4 print:gap-1 mb-4 print:mb-1">
                    ...
                    <div className="flex gap-6 print:gap-2 mt-2">
        ...
        <div className="flex flex-col gap-8 print:gap-2 print:gap-3 print:p-3">
            ...
            <div className="overflow-x-auto p-1">
                <table className="w-full border-collapse border-4 border-zinc-800 text-xs bg-white shadow-xl rounded-2xl">
                    <thead>
                        <tr>
                            <th className="border-2 border-zinc-800 bg-zinc-100 p-4 print:p-1"></th>
                    ...
                    <tbody>
                        {(data?.people || []).map((person, i) => (
                            <tr key={i}>
                                <td className="border-2 border-zinc-800 p-4 print:p-1 font-black bg-zinc-100 uppercase text-zinc-700"><EditableText value={person} tag="span" /></td>
...
const SimpleVisualList = ({ items, title }: { items: any[], title: string }) => (
    <div className="space-y-4">
        {(items || []).map((item: any, i: number) => (
            <EditableElement key={i} className="border-2 border-zinc-100 bg-white rounded-2xl p-4 print:p-1 flex justify-between items-center break-inside-avoid item-card h-20 shadow-sm">
                <div className="font-black text-indigo-600 flex items-center gap-4 print:gap-1">
...
export const FutoshikiSheet = ({ data }: { data: FutoshikiData }) => (
    <div className="flex flex-col">
        ...
        <div className="dynamic-grid mt-6 print:mt-2">
            {(data?.puzzles || []).map((p, i) => (
                <div key={i} className="border-4 border-zinc-800 aspect-square flex items-center justify-center relative item-card p-6 print:p-2 bg-white shadow-lg rounded-[2.5rem] break-inside-avoid">
...
export const NumberPyramidSheet = ({ data }: { data: NumberPyramidData }) => (
    <div className="flex flex-col">
        <PedagogicalHeader title={data?.title} instruction={data?.instruction} data={data} />
        <div className="dynamic-grid mt-10 print:mt-3">
            {(data?.pyramids || []).map((p, i) => (
                <div key={i} className="flex flex-col items-center gap-2 break-inside-avoid item-card justify-center py-8 print:py-2">
                    {(p?.rows || []).map((row, r) => (
...
export const OddOneOutSheet = ({ data }: { data: OddOneOutData }) => (
    <div className="flex flex-col">
        ...
        <div className="space-y-4">
            {(data?.groups || []).map((g, i) => (
                <EditableElement key={i} className="border-2 border-zinc-100 p-6 print:p-2 rounded-2xl text-xl flex gap-6 print:gap-2 justify-center flex-wrap item-card items-center bg-white shadow-sm break-inside-avoid">
                    <span className="font-black text-zinc-300 w-6">{i + 1}.</span>
                    {(g?.words || []).map((w, j) => (
                        <div key={j} className="px-6 print:px-2 py-2 border-2 border-zinc-100 bg-zinc-50 rounded-xl font-black text-zinc-700 hover:border-indigo-400 cursor-pointer transition-all">
