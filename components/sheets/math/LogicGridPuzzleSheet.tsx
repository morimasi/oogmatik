...
export const LogicGridPuzzleSheet = ({ data }: { data: LogicGridPuzzleData }) => (
    <div className="flex flex-col font-lexend">
        ...
        <div className="flex flex-col gap-10 print:gap-3 print:gap-4 print:gap-1 print:p-4 print:p-1 mt-4 print:mt-1">
            <EditableElement className="text-base border-[3px] border-zinc-900 p-8 print:p-2 print:p-3 bg-zinc-50 rounded-[3rem] shadow-sm">
                <h4 className="font-black text-[10px] uppercase tracking-[0.4em] text-indigo-600 mb-6 print:mb-2 flex items-center gap-3">
                    <i className="fa-solid fa-lightbulb animate-pulse"></i> İPUÇLARI VE VERİLER
                </h4>
                <ul className="space-y-4">
                    {(data?.clues || []).map((clue, i) => (
                        <li key={i} className="flex gap-4 print:gap-1 text-zinc-700 leading-relaxed font-bold">
            ...
            <div className="overflow-x-auto p-2">
                <table className="w-full border-collapse border-[4px] border-zinc-900 text-xs bg-white shadow-2xl rounded-[2rem] overflow-hidden">
                    <thead>
                        <tr>
                            <th className="border-2 border-zinc-900 bg-zinc-100 p-6 print:p-2"></th>
                    ...
                    <tbody>
                        {(data?.people || []).map((person, i) => (
                            <tr key={i}>
                                <td className="border-2 border-zinc-900 p-6 print:p-2 font-black bg-zinc-100 uppercase text-zinc-700 text-sm"><EditableText value={person} tag="span" /></td>
