...
export const VisualInterpretationConfig: React.FC<VisualInterpretationConfigProps> = ({ options, setOptions }) => {
    return (
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select
                        ...
                        onChange={(e) => setOptions({ ...options, questionStyle: e.target.value as any })}
                        className="w-full bg-zinc-100 dark:bg-zinc-800 border-none rounded-xl p-3 text-sm font-medium focus:ring-2 focus:ring-indigo-500"
                    >
            ...
            <div className="space-y-3 pt-2 border-t border-zinc-100 dark:border-zinc-800">
                <label className="flex items-center gap-3 p-3 bg-zinc-50 dark:bg-zinc-900/50 rounded-xl cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
                    <input
                        ...
                        onChange={(e) => setOptions({ ...options, includeClinicalNotes: e.target.checked })}
                        className="w-5 h-5 rounded text-indigo-600 focus:ring-indigo-500 border-zinc-300"
                    />
                    <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                        Klinik Gözlem Notları Ekle
                        <span className="block text-xs text-zinc-400 font-normal">Öğrencinin dikkat ve algı performansını not almak için alan bırakır.</span>
                    </span>
                </label>
                ...
                <label className="flex items-center gap-3 p-3 bg-zinc-50 dark:bg-zinc-900/50 rounded-xl cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">
                    <input
                        ...
                        onChange={(e) => setOptions({ ...options, syllableColoring: e.target.checked })}
                        className="w-5 h-5 rounded text-indigo-600 focus:ring-indigo-500 border-zinc-300"
                    />
                    <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300">
                        Soruları Renkli Heceyle Yaz
                        <span className="block text-xs text-zinc-400 font-normal">Disleksi dostu okuma desteği sağlar.</span>
                    </span>
                </label>
            </div>
        </div>
