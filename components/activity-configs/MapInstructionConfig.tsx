    ...
    return (
        <div className="space-y-5 animate-in fade-in duration-300">
            <div className="p-4 bg-blue-50/50 dark:bg-blue-900/10 rounded-[2rem] border border-blue-100 dark:border-blue-800/30">
                <label className="text-[10px] font-black text-blue-600 uppercase mb-3 block text-center">Bölgesel Odak</label>
                <div className="grid grid-cols-2 gap-2">
                    {regions.map(r => (
                        <button 
                            key={r.value}
                            onClick={() => onChange('emphasizedRegion', r.value)}
                            className={`py-2 rounded-xl text-[10px] font-black border transition-all ${options.emphasizedRegion === r.value ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-zinc-800 text-zinc-500 border-zinc-200 dark:border-zinc-700'}`}
                        >
                            {r.label}
                        </button>
                    ))}
