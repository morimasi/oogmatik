...
export const OperationCardVertical: React.FC<OperationCardProps> = ({ op, fontSize, showText, themeConfig, index }) => {
    const numStr = NUMBERING_STYLES[themeConfig.numberingStyle].format(index + 1);

    return (
        <div className={`flex flex-col items-end font-mono font-bold leading-none break-inside-avoid relative ${getBorderClass(themeConfig.borderStyle)}`} style={{ fontSize: `${fontSize}px` }}>
            {themeConfig.numberingStyle !== 'none' && (
                <span className="absolute -left-4 -top-2 text-[0.4em] text-zinc-400 font-sans">{numStr}.</span>
            )}
...
export const OperationCardHorizontal: React.FC<OperationCardProps> = ({ op, fontSize, showText, themeConfig, index }) => {
    const numStr = NUMBERING_STYLES[themeConfig.numberingStyle].format(index + 1);

    return (
        <div className={`flex flex-wrap items-center gap-2 font-mono font-bold break-inside-avoid relative ${getBorderClass(themeConfig.borderStyle)}`} style={{ fontSize: `${fontSize}px` }}>
            {themeConfig.numberingStyle !== 'none' && (
                <span className="absolute -left-3 -top-3 text-[0.4em] text-zinc-400 font-sans">{numStr}.</span>
            )}
