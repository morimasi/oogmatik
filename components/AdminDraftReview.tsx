...
export const AdminDraftReview: React.FC = () => {
    ...
    const handlePublish = async () => {
        ...
        try {
            await adminService.publishDraft(selectedDraft, pubConfig);
            setDrafts(prev => prev.filter(d => d.id !== selectedDraft.id));
            setSelectedDraft(null);
            alert("Etkinlik başarıyla yayınlandı! 'Diğerleri' kategorisinde görünecektir.");
        } catch (e) {
                    ...
                    <div className="p-8 max-w-3xl mx-auto w-full">
                        ...
                        <div className="bg-indigo-50 dark:bg-indigo-900/20 p-6 rounded-2xl border border-indigo-100 dark:border-indigo-800 mb-8">
                            ...
                            <div className="mt-4">
                                ...
                                <p className="text-xs text-zinc-600 dark:text-zinc-400 bg-white dark:bg-black p-3 rounded border border-indigo-100 dark:border-indigo-900">{selectedDraft.customInstructions}</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select value={pubConfig.category} onChange={e => setPubConfig({...pubConfig, category: e.target.value})} className="w-full p-3 border rounded-xl bg-zinc-50 dark:bg-zinc-800 dark:border-zinc-700 focus:ring-2 focus:ring-indigo-500 outline-none">
                                        {ACTIVITY_CATEGORIES.map(c => <option key={c.id} value={c.id}>{c.title}</option>)}
                                        <option value="others">Diğerleri (Özel)</option>
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-zinc-700 dark:text-zinc-300 mb-2">İkon</label>
                                    <div className="flex gap-2 flex-wrap bg-zinc-50 dark:bg-zinc-800 p-2 rounded-xl border dark:border-zinc-700">
                                        {ICON_LIST.map(icon => (
                                            <button 
                                                key={icon}
