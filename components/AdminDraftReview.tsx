...
const ICON_LIST = [
    'fa-wand-magic-sparkles', 'fa-book-open', 'fa-calculator', 'fa-brain',
    'fa-eye', 'fa-ear-listen', 'fa-pen-nib', 'fa-robot', 'fa-rocket',
    'fa-star', 'fa-shapes', 'fa-bolt', 'fa-puzzle-piece', 'fa-layer-group',
    'fa-cube', 'fa-flask', 'fa-dna', 'fa-microchip', 'fa-gamepad'
];
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar-minimal px-4 py-6 space-y-3">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center p-12 gap-3 opacity-30">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest">Veriler Okunuyor</span>
                        </div>
                    ) : drafts.length === 0 ? (
                        ...
                        drafts.map((item: ActivityDraft) => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-3">
                                    ...
                                    <span className="text-[9px] font-bold text-zinc-400">{new Date(item.createdAt).toLocaleDateString()}</span>
                                </div>
                                <h4 className="text-xs font-black text-zinc-800 dark:text-zinc-200 truncate group-hover:text-indigo-500 transition-colors uppercase leading-tight">{item.title || 'İsimsiz Analiz'}</h4>
                                <p className="text-[10px] text-zinc-500 mt-1 flex items-center gap-1 font-bold italic"><i className="fa-solid fa-user-circle"></i> {item.createdBy}</p>
                            </div>
                        ))
                    )}
                </div>
            ...
            <div className="flex-1 flex flex-col bg-white/50 dark:bg-zinc-950/20 overflow-hidden">
                {selectedDraft ? (
                    <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-500">
                        ...
                        <div className="p-8 border-b border-zinc-100 dark:border-white/5 flex justify-between items-center bg-white/80 dark:bg-zinc-900/40 backdrop-blur-xl">
                            ...
                            <div className="flex gap-3">
                                <button onClick={() => handleDelete(selectedDraft.id)} className="w-14 h-14 bg-red-500/10 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all transform active:scale-95"><i className="fa-solid fa-trash-can text-lg"></i></button>
                                <button onClick={handleRefine} disabled={isRefining} className={`px-10 h-14 bg-zinc-900 dark:bg-white text-white dark:text-black rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl transition-all flex items-center gap-3 ${isRefining ? 'animate-pulse opacity-50' : 'hover:scale-105 active:scale-95 shadow-indigo-500/20'}`}>
                                    {isRefining ? <i className="fa-solid fa-microchip fa-spin"></i> : <i className="fa-solid fa-wand-magic-sparkles"></i>}
                                    AI İLE YAPILANDIR
                                </button>
                                <button onClick={handlePublish} disabled={isPublishing || isRefining} className={`px-12 h-14 bg-emerald-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl transition-all flex items-center gap-3 ${isPublishing ? 'animate-pulse opacity-50' : 'hover:bg-emerald-500 hover:scale-105 active:scale-95 shadow-emerald-500/30'}`}>
                                    {isPublishing ? <i className="fa-solid fa-circle-notch fa-spin"></i> : <i className="fa-solid fa-rocket"></i>}
                                    YAYINLA
                                </button>
                            </div>
                        </div>
                            ...
                            <div className="max-w-6xl mx-auto grid grid-cols-1 xl:grid-cols-2 gap-12">
                                ...
                                <div className="space-y-8">
                                    ...
                                    <div className="space-y-4">
                                        <h5 className="text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">Kullanıcı Notları / Talimatları</h5>
                                        <div className="p-6 bg-indigo-50/50 dark:bg-indigo-900/10 rounded-2xl border border-indigo-100 dark:border-indigo-900/30">
                                            <p className="text-xs font-bold text-indigo-900 dark:text-indigo-300 italic leading-relaxed">"{selectedDraft.customInstructions || 'Talimat verilmedi.'}"</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-8">
                                    ...
                                    <div className="space-y-6 bg-white dark:bg-zinc-900 p-10 rounded-[3rem] shadow-2xl border-2 border-zinc-100 dark:border-white/5 relative">
                                        ...
                                        <div className="space-y-4">
                                            <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">EĞİTİM BAŞLIĞI</label>
                                            <input
                                                type="text"
                                        ...
                                        <div className="grid grid-cols-2 gap-6">
                                            <div className="space-y-4">
                                                <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">KATEGORİ</label>
                                                <select
                                            ...
                                            <div className="space-y-4">
                                                <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">SİMGE</label>
                                                <div className="relative group">
                                                    <div className="absolute left-6 top-1/2 -translate-y-1/2 text-lg text-indigo-500"><i className={`fa-solid ${refinedData.icon || 'fa-star'}`}></i></div>
                                                    <select
                                        ...
                                        <div className="space-y-4">
                                            <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">KLİNİK AÇIKLAMA</label>
                                            <textarea
                                    ...
                                    <div className="p-8 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-[2.5rem] shadow-xl text-white flex items-center justify-between group overflow-hidden relative">
                                        ...
                                        <div className="relative z-10">
                                            <h5 className="font-black text-lg uppercase tracking-tight mb-1">MİMARİ HAZIR</h5>
                                            <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest">Yayınla butonu ile sisteme entegre edilecektir.</p>
                                        </div>
                                        <div className="text-4xl opacity-50 group-hover:scale-110 transition-transform"><i className="fa-solid fa-check-double"></i></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center p-24 text-center">
                        ...
                        <p className="text-sm text-zinc-500 font-bold max-w-sm leading-relaxed uppercase tracking-tight opacity-50">Lütfen bekleyen OCR taslaklarından birini seçerek analitik süreci başlatın.</p>
                    </div>
                )}
            </div>
        </div>
