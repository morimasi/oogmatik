...
const MenuButton = ({ icon, label, onClick, active, isOpen }: any) => (
    ...
    >
        <i className={`fa-solid ${icon} ${active || isOpen ? 'text-[var(--accent-color)]' : 'text-[var(--text-muted)]'}`}></i>
        <span className="hidden xl:inline">{label}</span>
        <i className={`fa-solid fa-chevron-down text-[10px] ml-1 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}></i>
    </button>
);
...
const NumberControl = ({ label, value, onChange, min, max, step = 1, unit = '' }: any) => (
    <div className="flex items-center justify-between py-1">
        ...
        <div className="flex items-center gap-2 bg-[var(--bg-secondary)] rounded-md p-0.5 border border-[var(--border-color)]">
            <button onClick={() => onChange(Math.max(min, value - step))} className="w-6 h-6 flex items-center justify-center text-[var(--text-secondary)] hover:bg-[var(--surface-glass)] rounded shadow-sm transition-all" disabled={value <= min}><i className="fa-solid fa-minus text-[10px]"></i></button>
            <span className="text-xs font-mono font-bold w-10 text-center text-[var(--text-primary)]">{Math.round(value * 100) / 100}{unit}</span>
            <button onClick={() => onChange(Math.min(max, value + step))} className="w-6 h-6 flex items-center justify-center text-[var(--text-secondary)] hover:bg-[var(--surface-glass)] rounded shadow-sm transition-all" disabled={value >= max}><i className="fa-solid fa-plus text-[10px]"></i></button>
        </div>
    </div>
);
...
const Toggle = ({ label, checked, onChange }: any) => (
    <div className="flex items-center justify-between py-1 cursor-pointer group" onClick={() => onChange(!checked)}>
        ...
        <div className={`w-8 h-4 rounded-full relative transition-colors ${checked ? 'bg-[var(--accent-color)]' : 'bg-[var(--border-color)]'}`}>
...
const DropdownPanel = ({ title, children, onClose, className = "" }: any) => {
    ...
    return (
        <div ref={ref} className={`absolute top-full mt-2 left-0 bg-[var(--panel-bg)] border border-[var(--border-color)] rounded-xl shadow-2xl z-50 p-5 animate-in fade-in zoom-in-95 origin-top-left backdrop-blur-xl ring-1 ring-white/5 ${className}`}>
            <div className="flex justify-between items-center mb-4 pb-3 border-b border-[var(--border-color)]">
                <h4 className="text-[10px] font-black text-[var(--accent-color)] uppercase tracking-[0.2em]">{title}</h4>
                <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors"><i className="fa-solid fa-times"></i></button>
            </div>
    ...
    return (
        <div className="flex items-center justify-between gap-2 h-12 select-none relative">
            <div className="flex items-center gap-2">
                ...
                <div className="relative">
                    <MenuButton icon="fa-table-cells-large" label="Sayfa Düzeni" isOpen={activeMenu === 'layout'} onClick={() => setActiveMenu(activeMenu === 'layout' ? null : 'layout')} />
                    {activeMenu === 'layout' && (
                        <DropdownPanel title="Mizanpaj Ayarları" onClose={() => setActiveMenu(null)} className="w-64">
                            <Toggle label="Etkinlik Başlığını Göster" checked={settings.showTitle !== false} onChange={(v: any) => updateSetting('showTitle', v)} />
                            <Toggle label="Yönergeyi Göster" checked={settings.showInstruction !== false} onChange={(v: any) => updateSetting('showInstruction', v)} />
                        </DropdownPanel>
                    )}
                </div>
            </div>
            ...
            <div className="flex items-center gap-2">
                ...
                {onShare && <IconButton icon="fa-share-nodes" title="Paylaş" onClick={onShare} colorClass="bg-[var(--accent-color)] !text-[var(--bg-primary)] hover:!bg-[var(--accent-hover)]" />}
            </div>
        </div>
    );
};
