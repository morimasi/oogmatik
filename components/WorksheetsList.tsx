...
export const WorksheetsList: React.FC<WorksheetsListProps> = ({
    userId,
    userRole,
    userName,
}) => {
    ...
    // API hooks
    const { data: worksheetsData, loading, error } = useGetUserWorksheets({
        userId,
        userRole,
        page,
        pageSize,
    ...
    const { deleteWorksheet, loading: deleteLoading, error: deleteError } = useDeleteWorksheet(userId, userRole);
    const { shareWorksheet, loading: shareLoading, error: shareError } = useShareWorksheet(userId, userRole);
                                ...
                                <tr key={worksheet.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-gray-600">
                                        {worksheet.userId === userId ? (
                                            ...
                                            <span className="text-gray-500">{worksheet.sharedByName || 'Paylaşılan'}</span>
                                        )}
                                    </td>
                                    <td className="px-4 py-3 text-gray-600">
            ...
            {worksheetsData && worksheetsData.count > pageSize && (
                <div className="mt-6 flex items-center justify-between">
                    <p className="text-sm text-gray-600">
                        Toplam: {worksheetsData.count} çalışma
                    </p>
