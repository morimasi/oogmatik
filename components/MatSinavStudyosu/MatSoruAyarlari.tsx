        ...
        <div className="space-y-4">
            ...
            <div className="bg-gradient-to-r from-red-500 to-orange-500 p-[2px] rounded-2xl shadow-sm hover:shadow-md transition-all">
                <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-3 rounded-[14px] transition-all ${ayarlar.isLgsMode ? 'bg-gradient-to-r from-red-500 to-orange-500 text-white' : 'bg-white text-[var(--text-primary)] hover:bg-orange-50'}`}
                >
                    <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                            <span className={`text-[13px] font-black block tracking-tight ${ayarlar.isLgsMode ? 'text-white' : 'text-[var(--text-primary)]'}`}>LGS Yeni Nesil Denemesi</span>
                            <span className={`text-[10px] font-medium ${ayarlar.isLgsMode ? 'text-white/80' : 'text-[var(--text-muted)]'}`}>8. Sınıf • 20 Soru • Zor Seviye</span>
                        </div>
                    </div>
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${ayarlar.isLgsMode ? 'border-white bg-white text-orange-500' : 'border-[var(--border-color)]'}`}>
            ...
            <div className="space-y-2">
                {SORU_TIPLERI.map(({ key, label, icon, desc }) => (
                    <div key={key} className="group flex items-center justify-between bg-white rounded-2xl px-4 py-3 border border-[var(--border-color)] transition-all duration-200 hover:border-accent/20 hover:shadow-sm">
                        <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="truncate">
                                <span className="text-[13px] font-bold text-[var(--text-primary)] block tracking-tight">{label}</span>
                                <span className="text-[10px] text-[var(--text-muted)] font-medium">{desc}</span>
                            </div>
                        </div>
                        <div className="flex items-center gap-1 bg-[var(--bg-secondary)] p-1 rounded-xl border border-[var(--border-color)]">
                            ...
                            <span className="w-7 text-center text-[13px] font-black text-[var(--text-primary)]">
                ...
                {/* Toplam Gösterge */}
                <div className={`mt-4 p-4 rounded-2xl border transition-all duration-300 ${toplamSoru >= 1 ? 'bg-[var(--bg-secondary)] border-[var(--border-color)]' : 'bg-rose-50 border-rose-100'}`}>
                    <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                            <span className={`text-[10px] font-bold uppercase tracking-wider ${toplamSoru >= 1 ? 'text-[var(--text-muted)]' : 'text-rose-600'}`}>Toplam Soru</span>
                            <span className="text-lg font-black text-[var(--text-primary)]">{toplamSoru} Soru</span>
                        </div>
                        {toplamSoru >= 1 ? (
            ...
            <div>
                <div className="flex items-center gap-2 mb-3 px-1">
                    <span className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Zorluk Prerogatifleri</span>
                    <div className="h-px flex-1 bg-[var(--bg-secondary)]"></div>
                </div>
                <div className="grid grid-cols-4 gap-1 p-1 bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-color)]">
                    {ZORLUK_SEVIYELERI.map((sev) => (
                        <button
                            ...
                            className={`py-2 rounded-lg text-[10px] font-black transition-all duration-300 ${ayarlar.zorlukSeviyesi === sev
                                ...
                                : 'text-[var(--text-muted)] hover:text-[var(--text-muted)] hover:bg-white/50'
                                }`}
                        >
            ...
            <div>
                <div className="flex items-center gap-2 mb-3 px-1">
                    <span className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">İşlem Filtresi</span>
                    <div className="h-px flex-1 bg-[var(--bg-secondary)]"></div>
                </div>
                <div className="grid grid-cols-4 gap-1 p-1 bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-color)]">
            ...
            {/* Görsel Veri Toggle */}
            <div className="group flex items-center justify-between bg-white rounded-2xl px-4 py-3 border border-[var(--border-color)] transition-all duration-200 hover:border-accent/20 shadow-sm">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-[var(--bg-secondary)] flex items-center justify-center text-xl transition-colors group-hover:bg-accent/10">📊</div>
                    <div>
                ...
                <button
                    ...
                    className={`relative w-11 h-6 rounded-full transition-all duration-300 ${ayarlar.gorselVeriEklensinMi ? 'bg-accent shadow-[0_0_12px_hsl(var(--accent-h)_var(--accent-s)_var(--accent-l)_/_0.3)]' : 'bg-[var(--bg-secondary)]'
                        }`}
                >
            ...
            <div className="space-y-3">
                <div className="flex items-center gap-2 mb-1 px-1">
                    <span className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">İçerik Teması</span>
                    <div className="h-px flex-1 bg-[var(--bg-secondary)]"></div>
                </div>
