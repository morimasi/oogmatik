...
// Fix: Define GeneratorViewProps interface missing on line 85
...
export const GeneratorView: React.FC<GeneratorViewProps> = ({ activity, onGenerate, onBack, isLoading, isExpanded = true, onOpenStudentModal }) => {
    ...
    const [options, setOptions] = useState<GeneratorOptions>({
        ...
        worksheetCount: 1,
        itemCount: 10,
        variant: 'mixed', // focusSide
        familyDepth: 'extended',
        // Fix: Use a valid logicModel value from the expanded union on line 95
        logicModel: 'sequence' // replaced 'indirect' with 'sequence' for default compatibility or just ensure it is in union
    });
    ...
    const renderActivityControls = () => {
        if (activity.id === ActivityType.FAMILY_LOGIC_TEST) {
            return (
                <div className="space-y-5 animate-in fade-in duration-300">
                    <CompactToggleGroup 
                        label="Soy Odak Tarafı" 
                    ...
                    <div className="p-4 bg-zinc-50 dark:bg-zinc-800 rounded-2xl border border-zinc-100 dark:border-zinc-700 space-y-4">
                        <CompactSelect 
                            label="Akrabalık Derinliği" 
                            value={options.familyDepth} 
                            onChange={(v:string) => handleChange('familyDepth', v)}
                            options={[
                                { value: 'basic', label: 'Temel (Anne/Baba/Kardeş)' },
                                { value: 'extended', label: 'Geniş (Hala/Teyze/Kuzen)' },
                                { value: 'expert', label: 'Uzman (Elti/Görümce/Bacanak)' }
                            ]}
                            icon="fa-sitemap"
                        />

                        <CompactSelect 
                            label="Mantıksal Seviye" 
                            value={options.logicModel} 
                            onChange={(v:string) => handleChange('logicModel', v)}
                            options={[
                                { value: 'simple', label: 'Doğrudan İlişki' },
                                { value: 'indirect', label: 'Dolaylı Çıkarım' },
                                { value: 'syllogism', label: 'Karmaşık Kıyas (Eğer...)' }
                            ]}
                            icon="fa-brain"
                        />

                        <CompactCounter label="İfade Sayısı" value={options.itemCount} onChange={(v:number) => handleChange('itemCount', v)} min={4} max={15} icon="fa-list-ol" />
                    </div>
                    ...
                    <div className="p-3 bg-amber-50 dark:bg-amber-900/10 rounded-xl border border-amber-100 dark:border-amber-800/30">
                        <p className="text-[9px] font-black text-amber-600 uppercase tracking-widest mb-1 flex items-center gap-1">
                            <i className="fa-solid fa-lightbulb"></i> Bilişsel Hedef
                        </p>
                        <p className="text-[10px] text-zinc-500 italic leading-tight">Bu ayarlar soyut hiyerarşik yapıları anlama ve sosyal muhakeme derinliğini belirler.</p>
                    </div>
                </div>
            );
        }
        ...
        <div className="flex flex-col h-full bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-700 shadow-xl overflow-hidden w-full transition-all duration-300">
            ...
            <div className={`flex-1 overflow-y-auto p-4 custom-scrollbar min-h-0 transition-opacity duration-200 ${isExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                <div className="grid grid-cols-2 gap-3 mb-6">
                    <CompactCounter label="Sayfa" value={options.worksheetCount} onChange={(v: number) => handleChange('worksheetCount', v)} min={1} max={5} icon="fa-copy" />
                ...
                <div className="mt-8 space-y-3">
                    <button 
                        ...
                        disabled={isLoading}
                        className="w-full px-4 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-black text-xs shadow-lg shadow-indigo-500/20 transition-all flex items-center justify-center gap-2 transform active:scale-95"
                    >
                        <i className="fa-solid fa-wand-magic-sparkles"></i> AI İLE DERİN ÜRETİM
                    </button>
                    <button 
                        onClick={() => onGenerate({ ...options, mode: 'fast' })}
                        disabled={isLoading}
                        className="w-full py-3 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-800 dark:text-zinc-200 rounded-xl font-bold text-xs transition-all flex items-center justify-center gap-2"
                    >
                        <i className="fa-solid fa-bolt"></i> HIZLI MOD
                    </button>
                </div>
            </div>
        </div>
