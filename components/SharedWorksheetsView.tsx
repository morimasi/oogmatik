...
export const SharedWorksheetsView: React.FC<SharedWorksheetsViewProps> = ({ onLoad, onBack }) => {
  ...
  const loadShared = async () => {
      if (user) {
          const [worksheets, assessments] = await Promise.all([
              worksheetService.getSharedWithMe(user.id),
              assessmentService.getSharedAssessments(user.id)
          ]);
          setSharedWorksheets(worksheets);
  ...
  const groupedData = useMemo(() => {
    ...
    // Add Assessments as a specific group if exists
    if (sharedAssessments.length > 0) {
        grouped['assessments'] = {
  ...
  }, [sharedWorksheets, sharedAssessments]);
