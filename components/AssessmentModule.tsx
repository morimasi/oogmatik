...
const DOMAINS: { id: CognitiveDomain; title: string; desc: string; icon: string; estimatedTime: string }[] = [
    { id: 'visual_spatial_memory', title: 'Görsel-Uzamsal Bellek (Matrix)', desc: 'Kısa süreli görsel hafıza ve desen takibi.', icon: 'fa-table-cells', estimatedTime: '3 dk' },
    { id: 'processing_speed', title: 'Hızlı İsimlendirme (RAN)', desc: 'Görsel uyaranları işlemleme ve sözel tepki hızı.', icon: 'fa-stopwatch', estimatedTime: '2 dk' },
    { id: 'selective_attention', title: 'Stroop Testi (Dikkat)', desc: 'Dürtü kontrolü, odaklanma ve çeldirici baskılama.', icon: 'fa-traffic-light', estimatedTime: '3 dk' },
...
export const AssessmentModule: React.FC<AssessmentModuleProps> = ({ onBack, onSelectActivity, onAddToWorkbook, onAutoGenerateWorkbook }) => {
    ...
    // Sync with global active student if already selected
    useEffect(() => {
    ...
    const generateFinalReport = async (completedResults: SubTestResult[]) => {
        ...
        completedResults.forEach(res => {
            if (res.score < 65) {
                ...
                suggested.forEach(actId => {
                    ...
                    if (def) roadmap.push({ activityId: actId, title: def.title, reason: `${res.name} desteği için.`, frequency: 'Haftada 3 kez', priority: 'medium' });
                });
            }
        });
        ...
        const reportData: ProfessionalAssessmentReport = {
            ...
            overallRiskAnalysis: {
                ...
                summary: `Öğrenci ${studentName}, batarya genelinde %${Math.round(totalScore)} performans göstermiştir.`
            },
        ...
        const fullReport: any = {
            professionalData: reportData,
            overallSummary: reportData.overallRiskAnalysis.summary,
            scores: { attention: attentionScore, spatial: memoryScore, logical: logicResult?.score || 0, linguistic: processingResult?.score || 0 },
        ...
        if (user) {
            await assessmentService.saveAssessment(user.id, studentName, 'Erkek', studentAge, activeStudent?.grade || '1. Sınıf', fullReport, studentId);
        }
        ...
        return (
            <div className="max-w-5xl mx-auto p-6 md:p-12 animate-in fade-in">
                <div className="flex items-center gap-4 mb-8">
                    <button onClick={onBack} className="w-10 h-10 rounded-xl bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center transition-colors text-zinc-500">
                        <i className="fa-solid fa-arrow-left"></i>
                    </button>
                    <div>
                        <h1 className="text-3xl font-black text-zinc-900 dark:text-white">Bilişsel Değerlendirme</h1>
                        <p className="text-zinc-500">Öğrenci profilini seçin ve analizi başlatın.</p>
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        <div className="bg-white dark:bg-zinc-800 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-700 shadow-sm">
                            <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                                <i className="fa-solid fa-user-graduate text-indigo-500"></i> Öğrenci Seçimi
                            </h3>
                            {students.length > 0 && (
                    ...
                    <div className="bg-zinc-900 dark:bg-black text-white p-8 rounded-3xl flex flex-col justify-between shadow-2xl relative overflow-hidden">
                        ...
                        <div>
                            ...
                            <p className="text-white/60 text-sm leading-relaxed mb-8">
                                {studentId ? `${studentName} için kişiselleştirilmiş analizi başlatıyorsunuz. Veriler öğrenci profiline kaydedilecektir.` : "Yeni bir öğrenci için bilişsel değerlendirme başlatıyorsunuz."}
                            </p>
                            <div className="space-y-4 mb-8">
                                <div className="flex items-center gap-3 text-sm text-white/80"><i className="fa-regular fa-clock w-5 text-center"></i><span>Toplam Süre: ~{selectedDomains.length * 3} Dakika</span></div>
                                <div className="flex items-center gap-3 text-sm text-white/80"><i className="fa-solid fa-graduation-cap w-5 text-center"></i><span>Hedef: Bireysel Gelişim Rotası</span></div>
                            </div>
                        </div>
                        <button onClick={startBattery} disabled={selectedDomains.length === 0} className="w-full py-4 bg-white text-black font-black rounded-xl hover:bg-zinc-200 transition-colors flex items-center justify-center gap-3 shadow-xl active:scale-95 disabled:opacity-50"><i className="fa-solid fa-play"></i> TESTİ BAŞLAT</button>
                    </div>
                </div>
            </div>
        );
            ...
            <div className="fixed inset-0 z-50 bg-zinc-50 dark:bg-zinc-900 flex flex-col">
                <div className="h-16 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-800 flex items-center justify-between px-6 shadow-sm">
                    <div className="flex items-center gap-4">
                        <div className="w-8 h-8 rounded-lg bg-indigo-100 text-indigo-600 flex items-center justify-center"><i className={`fa-solid ${domainInfo?.icon}`}></i></div>
                        <div>
