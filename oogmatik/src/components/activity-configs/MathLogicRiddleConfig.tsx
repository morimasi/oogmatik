 ...
 */
const ConfigSection = ({ title, children }: { title: string; children?: any }) => (
...
export const MathLogicRiddleConfig = ({ options, onChange }: { options: GeneratorOptions; onChange: (k: string, v: any) => void }) => {
    return (
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            onChange={(e: any) => onChange('itemCount', parseInt(e.target.value))}
                    ...
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            onChange={(e: any) => onChange('gridSize', parseInt(e.target.value))}
                            className="w-full h-1.5 bg-zinc-200 dark:bg-zinc-700 rounded-lg appearance-none cursor-pointer accent-indigo-600"
                        />
                        <div className="flex justify-between text-[8px] text-zinc-400 font-bold uppercase tracking-tighter">
                            <span>Basit (2)</span>
            ...
            <ConfigSection title="Klinik Özellikler">
                <div className="space-y-2">
                    <label className="flex items-center justify-between p-2 rounded-xl border border-transparent hover:bg-white dark:hover:bg-zinc-700 cursor-pointer transition-all">
                        ...
                        <div className={`w-8 h-4 rounded-full relative transition-colors ${(options as any).showSumTarget !== false ? 'bg-emerald-500' : 'bg-zinc-300'}`} onClick={() => onChange('showSumTarget', (options as any).showSumTarget === false)}>
                            <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${(options as any).showSumTarget !== false ? 'left-4.5' : 'left-0.5'}`}></div>
                        </div>
                    </label>
                    <label className="flex items-center justify-between p-2 rounded-xl border border-transparent hover:bg-white dark:hover:bg-zinc-700 cursor-pointer transition-all">
                        <span className="text-[10px] font-bold text-zinc-600 dark:text-zinc-300 uppercase">Görsel Destek (Rakamlar)</span>
                        <div className={`w-8 h-4 rounded-full relative transition-colors ${(options as any).showNumbers !== false ? 'bg-indigo-600' : 'bg-zinc-300'}`} onClick={() => onChange('showNumbers', (options as any).showNumbers === false)}>
                            <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${(options as any).showNumbers !== false ? 'left-4.5' : 'left-0.5'}`}></div>
                        </div>
                    </label>
                </div>
            </ConfigSection>
