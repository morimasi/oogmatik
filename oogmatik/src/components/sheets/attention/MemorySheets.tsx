...
export const WordMemorySheet = ({ data }: { data: WordMemoryData }) => (
    <div className="w-full font-lexend">
            ...
            <div className="flex-1 flex items-center justify-center">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6 print:gap-2 w-full max-w-2xl">
                    {(data.wordsToMemorize || []).map((word, i) => (
                        <div key={i} className="p-4 print:p-1 border-2 border-black rounded-xl text-center font-black text-lg uppercase">
        ...
        <div className="min-h-[500px] flex flex-col pt-12 print:pt-3 border-t-4 border-dashed border-zinc-100 mt-12 print:mt-3">
            <PedagogicalHeader title="TEST AŞAMASI" instruction="Az önce gördüğün kelimeleri işaretle." />
            <div className="grid grid-cols-3 md:grid-cols-4 gap-4 print:gap-1 mt-8 print:mt-2">
...
export const VisualMemorySheet = ({ data }: { data: VisualMemoryData }) => (
    <div className="w-full font-lexend">
        ...
        <div className="min-h-[500px] flex flex-col pt-12 print:pt-3 border-t-4 border-dashed border-zinc-100 mt-12 print:mt-3">
            ...
            <div className="grid grid-cols-4 md:grid-cols-5 gap-4 print:gap-1 mt-8 print:mt-2">
...
export const CharacterMemorySheet = ({ data }: { data: CharacterMemoryData }) => (
    <div className="w-full font-lexend">
            ...
            <div className="flex-1 grid grid-cols-2 md:grid-cols-3 gap-8 print:gap-2 print:gap-3 print:p-3 items-center justify-center p-4 print:p-1">
                {(data.charactersToMemorize || []).map((char, index) => (
                    <EditableElement key={index} className="flex flex-col items-center bg-white border-2 border-black p-4 print:p-1 rounded-3xl aspect-square justify-center shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                        <ImageDisplay prompt={char.imagePrompt} className="w-32 h-32 rounded-full object-cover border-4 border-black mb-4 print:mb-1" />
        ...
        <div className="min-h-[500px] flex flex-col pt-12 print:pt-3 border-t-4 border-dashed border-zinc-100 mt-12 print:mt-3">
            ...
            <div className="flex-1 grid grid-cols-3 md:grid-cols-4 gap-6 print:gap-2 p-4 print:p-1 mt-8 print:mt-2">
...
export const ImageComprehensionSheet = ({ data }: { data: ImageComprehensionData }) => (
    <div className="w-full font-lexend">
        ...
        <div className="space-y-6 mt-12 print:mt-3 pt-12 print:pt-3 border-t-2 border-dashed border-zinc-100">
            {(data.questions || []).map((q, i) => (
                <div key={i} className="p-6 print:p-2 border-2 border-zinc-900 rounded-3xl bg-white shadow-sm">
                    <p className="font-black text-lg mb-4 print:mb-1 flex gap-3"><span className="bg-zinc-900 text-white w-7 h-7 rounded-lg flex items-center justify-center text-sm">{i + 1}</span> {q}</p>
