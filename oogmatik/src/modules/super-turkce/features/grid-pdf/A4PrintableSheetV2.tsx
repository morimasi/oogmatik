...
const renderComponentByType = (type: string, draft: any, audience: string = 'normal') => {
  const data = draft.data ?? {}; // null/undefined'ı boş objeyle kapat

  // --- Disleksi Uyarlanmış Stil Yardımcıları ---
  const isDyslexic = audience === 'derin_disleksi' || audience === 'hafif_disleksi';
  const isDeepDyslexic = audience === 'derin_disleksi';
  const rowBgColor = (index: number) =>
    isDyslexic ? (index % 2 === 0 ? '#fefce8' : '#f0fdf4') : 'transparent'; // Sarı ve yeşil çok hafif bantlar
  const rowPadding = isDyslexic ? 4 : 0;
  const rowRadius = isDyslexic ? 4 : 0;
  ...
  const renderQuestionList = (questions: any[]) => (
    <View style={{ gap: 6, marginTop: 4 }}>
      {questions.map((q: any, i: number) => {
        const soruText =
          typeof q === 'string' ? q : safeText(q?.soru || q?.question, `Soru ${i + 1}`);
            ...
            {opts.length > 0 && (
              <View style={{ paddingLeft: 12, gap: 2 }}>
                {opts.map((opt: string, j: number) => (
                  <Text key={j} style={{ fontSize: 8, color: '#475569' }}>
                    {opt}
                  </Text>
                ))}
              </View>
    ...
    <View style={{ gap: 5, marginTop: 4 }}>
      {sentences.map((s: any, i: number) => (
        ...
        >
          {i + 1}.{' '}
          {safeText(
            typeof s === 'string' ? s : s?.cumle || s?.bozuk || s?.kelime || '',
            `Cümle ${i + 1}`
          )}
        </Text>
      ))}
  ...
  switch (type) {
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginTop: 5 }}>
          ...
          <View style={{ flex: 1, gap: 4 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap', marginTop: 4 }}>
              {(questions.length > 0 ? questions : ['Kim?', 'Ne?', 'Nerede?']).map(
                (q: any, i: number) => (
                  <Text key={i} style={{ fontSize: 8, color: '#0284c7', fontWeight: 'bold' }}>
                    {safeText(q, `S${i + 1}`)}
                  </Text>
                )
              )}
          ...
          {options.length > 0 && (
            <View style={{ gap: 3, paddingLeft: 10 }}>
              {options.map((o: string, i: number) => (
                <Text key={i} style={{ fontSize: 8, color: '#475569' }}>
                  {o}
                </Text>
              ))}
            </View>
    ...
    case 'METIN_KARSILASTIRMA': {
      const metin1 = safeText(data.metin1, 'Metin 1');
      const metin2 = safeText(data.metin2, 'Metin 2');
      const boyut = safeText(data.boyut, 'Karşılaştırma');
      return (
        <View style={{ gap: 6, marginTop: 4 }}>
          <View style={{ flexDirection: 'row', gap: 10 }}>
            <View style={{ flex: 1, backgroundColor: '#f8fafc', padding: 6, borderRadius: 4 }}>
              <Text style={{ fontSize: 8, fontWeight: 'bold', color: '#0f172a', marginBottom: 3 }}>
                Metin 1
              </Text>
              <Text style={{ fontSize: 8, color: '#334155' }}>{metin1}</Text>
            </View>
            <View style={{ flex: 1, backgroundColor: '#f8fafc', padding: 6, borderRadius: 4 }}>
              <Text style={{ fontSize: 8, fontWeight: 'bold', color: '#0f172a', marginBottom: 3 }}>
                Metin 2
              </Text>
              <Text style={{ fontSize: 8, color: '#334155' }}>{metin2}</Text>
            </View>
          </View>
          <View
            style={{
          ...
          >
            <Text style={{ fontSize: 8, color: '#64748b' }}>
              {boyut} analizi buraya yazılacak...
            </Text>
          </View>
        </View>
      );
    ...
    case 'KARAKTER_ANALIZI': {
      const karakterler = safeArray(data.karakterler);
      return (
        <View style={{ gap: 6, marginTop: 4 }}>
          {(karakterler.length > 0
            ? karakterler
            : [{ ad: 'Karakter', ozellikler: ['Özellik 1'] }]
          ).map((k: any, i: number) => (
            <View
              key={i}
              style={{
            ...
            >
              ...
              {safeArray(k.ozellikler).map((oz: any, j: number) => (
                <Text key={j} style={{ fontSize: 8, color: '#475569' }}>
                  • {safeText(oz)}
                </Text>
              ))}
          ...
          {(cumleler.length > 0 ? cumleler : [{ cumle: 'Cümle', sanat: 'Sanat' }]).map(
            (c: any, i: number) => (
              <View key={i} style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}>
                ...
                >
                  <Text style={{ fontSize: 8, color: '#94a3b8' }}>{safeText(c.sanat)}</Text>
                </View>
              </View>
            )
          )}
      ...
      return (
        <View style={{ gap: 5, marginTop: 4 }}>
          <Text style={{ fontSize: 8, color: '#64748b', marginBottom: 2 }}>
            Aşağıdaki cümleleri doğru sıraya koyunuz:
          </Text>
          {(shuffled.length > 0 ? shuffled : ['Cümle 1', 'Cümle 2', 'Cümle 3']).map(
            (s: any, i: number) => (
              <View key={i} style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}>
                <View
                  style={{
          ...
          {words.length > 0 && (
            ...
            >
              {words.map((w: string, i: number) => (
                <Text key={i} style={{ fontSize: 8, fontWeight: 'bold', color: '#0284c7' }}>
                  [{safeText(w)}]
                </Text>
              ))}
            </View>
    ...
    case 'BILMECELI_DUSUNME': {
      ...
      return (
        <View style={{ gap: 6, marginTop: 4 }}>
          ...
          <View style={{ gap: 3, paddingLeft: 10 }}>
            {(options.length > 0 ? options : ['A) ...', 'B) ...', 'C) ...', 'D) ...']).map(
              (opt: string, i: number) => (
                <Text key={i} style={{ fontSize: 9, color: '#475569' }}>
                  {safeText(opt)}
                </Text>
              )
            )}
    ...
    case 'KELIME_TAHMINI':
    case 'ULASMA': {
      const words = safeArray(
        data.words || data.kelimeler?.map((k: any) => safeText(k?.kelime || k, '')) || []
      );
    ...
    case 'MANTIKSIZLIGI_BUL': {
      ...
      return (
        <View style={{ gap: 5, marginTop: 4 }}>
          {(maddeler.length > 0 ? maddeler : ['İfade 1', 'İfade 2', 'İfade 3']).map(
            (m: any, i: number) => (
              <View key={i} style={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}>
                ...
                <Text style={{ fontSize: 9, color: '#334155', flex: 1 }}>
                  {safeText(
                    typeof m === 'string' ? m : m?.ifade || m?.cumle || m?.bozuk,
                    `İfade ${i + 1}`
                  )}
                </Text>
              </View>
    ...
    case 'RADYOAKTIF_HARF': {
      ...
      return (
        <View style={{ gap: 5, marginTop: 4 }}>
          {(kelimeler.length > 0 ? kelimeler : ['kelime1', 'kelime2', 'kelime3']).map(
            (k: any, i: number) => (
    ...
    case 'ACIK_UCLU': {
      ...
      if (type === 'OKUDUGUNU_CIZ') {
        const alanBoyutu = draft.settings?.boslukBoyutu || data.alanBoyutu || 'Orta';
        const height = alanBoyutu === 'Küçük' ? 80 : alanBoyutu.includes('Büyük') ? 250 : 140;
        const showLines = draft.settings?.yonlendirici !== false && data.cizgiSayisi !== 0;
    ...
    // ---- GÖRSEL VEKTÖREL (SVG) MODÜLLERİ ----
    case 'KAVRAM_HARITASI': {
      const merkez = safeText(data.merkez || data.merkezKelime, 'Merkez Kavram');
      const dallar = safeArray(data.dallar);
      // Basit 3-5 dallı SVG tabanlı ağ diyagramı
      return (
        <View style={{ alignItems: 'center', marginTop: 10, marginBottom: 10 }}>
          <Svg width="300" height="150" viewBox="0 0 300 150">
            ...
            <Rect
              ...
              strokeWidth="2"
            />
            <Text x="150" y="75" fill="#0f172a" fontSize="9" textAnchor="middle">
              {merkez}
            </Text>

            {/* Branches */}
            {dallar.map((dalObj: any, i: number) => {
              const total = Math.max(dallar.length, 3);
              const angle = Math.PI + (Math.PI / (total - 1)) * i; // 180 to 360 degrees (top half semicircle)
              // Calculate target positions
              const startX = 150;
              const startY = 55; // top of center box
              const radius = 60;
              const endX = 150 + Math.cos(angle) * radius;
              const endY = 55 + Math.sin(angle) * radius;

              const isBlank = typeof dalObj === 'object' ? dalObj.bos : false;
              const dalText = safeText(
                typeof dalObj === 'string' ? dalObj : dalObj?.dal,
                `Kavram ${i + 1}`
              );
              ...
              return (
                <React.Fragment key={i}>
                  ...
                  <Rect
                    ...
                    height="25"
                    rx="4"
                    fill={isBlank ? '#ffffff' : '#f1f5f9'}
                    stroke="#64748b"
                    strokeWidth="1"
                  />
                  {!isBlank && (
                    <Text x={endX} y={endY + 14} fill="#334155" fontSize="7" textAnchor="middle">
                      {dalText}
                    </Text>
                  )}
                </React.Fragment>
    ...
    case 'YONERGE_TAKIBI': {
      const yonlendirmeler = safeArray(data.yonlendirmeler || []);
      const isLabirent = data.alanTuru === 'labirent';

      return (
        <View style={{ gap: 8, marginTop: 4, flexDirection: 'row', alignItems: 'flex-start' }}>
          <View style={{ flex: 1, gap: 4 }}>
            {yonlendirmeler.map((ip: any, i: number) => (
              <Text key={i} style={{ fontSize: 9, color: '#334155' }}>
                {i + 1}. {safeText(ip, `Yönerge ${i + 1}`)}
              </Text>
            ))}
          </View>
          {isLabirent && (
            <View style={{ width: 120, height: 120, marginLeft: 10 }}>
              {/* 5x5 basit Grid / Labirent taslağı */}
              <Svg width="120" height="120" viewBox="0 0 120 120">
                ...
                {/* Başlangıç ve Bitiş */}
                <Rect x="2" y="2" width="20" height="20" fill="#10b981" rx="2" />
                <Text x="12" y="16" fill="#ffffff" fontSize="6" textAnchor="middle">
                  BAŞLA
                </Text>
                <Rect x="98" y="98" width="20" height="20" fill="#ef4444" rx="2" />
                <Text x="108" y="112" fill="#ffffff" fontSize="6" textAnchor="middle">
                  BİTİR
                </Text>
              </Svg>
            </View>
    ...
    case 'SOZEL_MANTIK_TABLO': {
      ...
      return (
        <View style={{ gap: 5, marginTop: 4 }}>
          {merkez ? (
            <Text
              style={{ fontSize: 10, fontWeight: 'bold', color: '#0f172a', textAlign: 'center' }}
            >
              {merkez}
            </Text>
          ) : null}
          ...
          {degiskenler.length > 0 ? (
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 4 }}>
              {kisiler.map((k: any, i: number) => (
                ...
                >
                  <Text style={{ fontSize: 8, fontWeight: 'bold', color: '#334155' }}>
                    {safeText(k, `Kişi ${i + 1}`)}
                  </Text>
                  {degiskenler.map((_: any, j: number) => (
                    <View
                      key={j}
                      style={{ width: '100%', height: 1, backgroundColor: '#f1f5f9', marginTop: 3 }}
                    />
                  ))}
                </View>
              ))}
              ...
              {kisiler.map((dal: any, i: number) => (
                ...
                >
                  <Text style={{ fontSize: 8, color: '#334155' }}>
                    {safeText(typeof dal === 'string' ? dal : dal?.dal, `Kavram ${i + 1}`)}
                  </Text>
                </View>
              ))}
            </View>
          ...
          {cumleler.length > 0 && (
            <View style={{ gap: 6 }}>
              {cumleler.map((c: any, i: number) => {
                const bozukMetin = safeText(
                  typeof c === 'string' ? c : c?.bozuk || c?.cumle,
                  `Cümle ${i + 1}`
                );
                ...
                return (
                  <View
                    key={i}
                    style={{ borderWidth: 1, borderColor: '#fecaca', borderRadius: 4, padding: 5 }}
                  >
          ...
          <View style={{ gap: 5, marginTop: 4 }}>
            {kurallar.map((k: any, i: number) => (
              <View key={i} style={{ flexDirection: 'row', gap: 6, alignItems: 'flex-start' }}>
                <Text style={{ fontSize: 8, fontWeight: 'bold', color: '#0284c7', width: 14 }}>
                  {i + 1}.
                </Text>
                <View style={{ flex: 1 }}>
                  ...
                  {k?.dogru && (
                    <Text style={{ fontSize: 8, color: '#16a34a' }}>✓ {safeText(k.dogru)}</Text>
                  )}
                  {k?.yanlis && (
                    <Text style={{ fontSize: 8, color: '#dc2626' }}>✗ {safeText(k.yanlis)}</Text>
                  )}
                  {k?.acilimi && (
                    <Text style={{ fontSize: 8, color: '#64748b' }}>{safeText(k.acilimi)}</Text>
                  )}
                  {k?.tanim && (
                    <Text style={{ fontSize: 8, color: '#475569' }}>{safeText(k.tanim)}</Text>
                  )}
                </View>
              </View>
            ))}
    ...
    case 'OZDEYIS_ANALIZ': {
      ...
      return (
        <View style={{ gap: 6, marginTop: 4 }}>
          {(deyimler.length > 0 ? deyimler : [{ deyim: 'Deyim', bosluk: '______' }]).map(
            (d: any, i: number) => (
              <View
                key={i}
                style={{ borderWidth: 1, borderColor: '#e2e8f0', borderRadius: 4, padding: 6 }}
              >
                <Text style={{ fontSize: 9, fontWeight: 'bold', color: '#0f172a' }}>
                  {safeText(d.deyim || d.ozdeyis, `${i + 1}. Deyim`)}
                </Text>
                {d.anlam && (
    ...
    case 'KAYNAŞTIRMA': {
      ...
      return (
        <View style={{ gap: 4, marginTop: 4 }}>
          {(ornekler.length > 0 ? ornekler : [{ temel: 'Örnek', ek: '(ek)' }]).map(
            (o: any, i: number) => (
              <View
                key={i}
                style={{
              ...
              >
                ...
                <Text style={{ fontSize: 8, color: '#64748b' }}>
                  {safeText(o?.ek || o?.dogru, '+ek')}
                </Text>
                <Text style={{ fontSize: 8, color: '#059669' }}>
                  → {safeText(o?.sonuc || o?.dogru || o?.olay, '...')}
                </Text>
              </View>
            )
          )}
      ...
      return (
        <View style={{ gap: 4, marginTop: 5 }}>
          ...
          <Text style={{ fontSize: 7, color: '#94a3b8', marginTop: 4 }}>
            ({safeText(draft.type, '?')} —{' '}
            {draft.settings?.engineMode === 'ai' ? 'AI ✨' : 'Hızlı ⚡'})
          </Text>
        </View>
      );
    }
    ...
    <Document title="Süper Türkçe Karma Çalışma Kağıdı">
      <Page size="A4" style={[styles.page, fontStyle]}>
        ...
        <View style={styles.header}>
          <View style={styles.brandContainer}>
            ...
            <Text style={styles.moduleName}>
              Süper Türkçe V2 -{' '}
              {selectedGrade ? `${selectedGrade}. Sınıf Çalışma Kağıdı` : 'Karma Etkinlik'}
            </Text>
          </View>
        ...
        <View style={{ marginBottom: 15 }}>
          <Text
            style={{ fontFamily: 'Lexend', fontSize: 10, fontWeight: 'bold', color: '#1e293b' }}
          >
            Ders Konusu:{' '}
            <Text style={{ fontWeight: 'normal' }}>
              {selectedObjective?.title || 'Genel Tekrar'}
            </Text>
          </Text>
        </View>
