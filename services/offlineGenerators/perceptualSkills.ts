...
export const generateOfflineAbcConnect = async (options: GeneratorOptions): Promise<AbcConnectData[]> => {
    const {itemCount, worksheetCount, gridSize, customLetters} = options;
    ...
    for(let i=0; i<worksheetCount; i++){
        ...
        if (customLetters) {
             const parts = customLetters.split(/[\s,]+/).map(s => s.trim().toUpperCase()).filter(s => s.length > 0);
             if (parts.length > 0) {
                 letters = parts;
             }
        }
        
        // Use cyclic indexing if we run out of custom letters for the required pairs
        const usedLetters = Array.from({ length: count }, (_, k) => letters[k % letters.length]);
        
        const placements = generateSmartConnectGrid(dim, count);
        
        const points = placements.map(p => ({
            label: usedLetters[p.pairIndex],
            x: p.x,
            y: p.y,
        ...
        results.push({
            ...
            instruction: "Aynı harfleri/sembolleri, çizgiler birbirini kesmeyecek şekilde birleştirin. Tüm kareler dolmalıdır.",
