...
const createShapePath = (type: 'triangle' | 'square' | 'circle' | 'line', cx: number, cy: number, size: number, rot: number = 0): string => {
    ...
    if (type === 'triangle') {
        for(let i=0; i<3; i++) {
            ...
            points.push({ x: cx + (size/2) * Math.cos(rad(angle)), y: cy + (size/2) * Math.sin(rad(angle)) });
        }
    } else if (type === 'square') {
        for(let i=0; i<4; i++) {
            const angle = rot + i * 90 - 45;
            points.push({ x: cx + (size/1.4) * Math.cos(rad(angle)), y: cy + (size/1.4) * Math.sin(rad(angle)) });
        }
    } else if (type === 'line') {
        const angle = rot;
        const len = size * 1.5;
        points.push({ x: cx + (len/2) * Math.cos(rad(angle)), y: cy + (len/2) * Math.sin(rad(angle)) });
        points.push({ x: cx - (len/2) * Math.cos(rad(angle)), y: cy - (len/2) * Math.sin(rad(angle)) });
...
export const generateOfflineGridDrawing = async (options: GeneratorOptions): Promise<GridDrawingData[]> => {
    ...
    return Array.from({ length: worksheetCount }, () => {
        ...
        return { title: 'Kare Çizim (Kopyalama)', instruction: 'Soldaki deseni sağdaki boş ızgaraya çizin.', pedagogicalNote: 'Görsel-uzamsal kopyalama ve ince motor.', imagePrompt: 'Grid Art', gridDim: dim, drawings };
    });
};
...
export const generateOfflineVisualOddOneOut = async (options: GeneratorOptions): Promise<VisualOddOneOutData[]> => {
    ...
    return Array.from({ length: worksheetCount }, () => {
        const rows = Array.from({ length: count }, () => {
            ...
            const correctIndex = shuffled.findIndex(x => (x as any).originalIdx === 3); 
...
export const generateOfflineStarHunt = async (options: GeneratorOptions): Promise<StarHuntData[]> => {
    ...
    return Array.from({ length: worksheetCount }, () => {
        ...
        return { title: `Yıldız Avı (${difficulty})`, instruction: difficulty === 'Başlangıç' ? 'Sadece içi dolu siyah yıldızları (★) bul.' : 'Gizlenmiş tüm yıldızları (★) bul. Benzer şekillere dikkat et!', pedagogicalNote: 'Şekil-Zemin ayrımı (Figure-Ground) ve seçici dikkat.', imagePrompt: 'Stars', grid, targetCount: count };
    });
};
...
export const generateOfflineShapeCounting = async (options: GeneratorOptions): Promise<ShapeCountingData[]> => {
    ...
    return Array.from({ length: worksheetCount }, () => {
        ...
        for(let i=0; i<numTargets; i++) {
            const cx = randomPos(); const cy = randomPos(); const size = randomSize();
            const rot = difficulty === 'Başlangıç' ? 0 : getRandomInt(0, 360);
            paths.push({ d: createShapePath('triangle', cx, cy, size, rot), fill: 'rgba(59, 130, 246, 0.5)', stroke: '#1e3a8a' });
        }
        for(let i=0; i<numDistractors; i++) {
            const type = Math.random() > 0.5 ? 'circle' : 'square';
            const cx = randomPos(); const cy = randomPos(); const size = randomSize();
            const rot = getRandomInt(0, 360);
            paths.push({ d: createShapePath(type, cx, cy, size, rot), fill: 'rgba(239, 68, 68, 0.5)', stroke: '#991b1b' });
        }
        for(let i=0; i<numNoiseLines; i++) {
            const cx = randomPos(); const cy = randomPos(); const size = randomSize() * 2;
            const rot = getRandomInt(0, 180);
            paths.push({ d: createShapePath('line', cx, cy, size, rot), fill: 'none', stroke: 'rgba(0,0,0,0.3)' });
        }
        return { title: `Şekil Sayma - Karışık (${difficulty})`, instruction: 'Sadece MAVİ ÜÇGENLERİ bul ve say. Üst üste gelmiş olabilirler.', pedagogicalNote: 'Şekil-Zemin algısı (Figure-Ground Perception) ve görsel ayrıştırma.', imagePrompt: 'Overlapping Shapes', figures: [{ targetShape: 'triangle', correctCount: numTargets, svgPaths: shuffle(paths) }] };
    });
};
