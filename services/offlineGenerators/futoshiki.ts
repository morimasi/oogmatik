...
export const generateOfflineFutoshiki = async (options: GeneratorOptions): Promise<FutoshikiData[]> => {
    ...
    // Boyut belirleme (Konfigürasyondan gelen değer öncelikli)
    let size = gridSize || 4;
    if (!gridSize) {
    ...
    for (let c = 0; c < worksheetCount; c++) {
        ...
        // İşaret yoğunluğu (density) parametresine göre ayarla
        ...
        // Başlangıç ipucu seviyesi (hintLevel) parametresine göre ayarla
        let emptyRatio = 0.6;
        if (hintLevel === 'low') emptyRatio = 0.85; // Daha az sayı kalsın (Zor)
        else if (hintLevel === 'high') emptyRatio = 0.3; // Daha çok sayı kalsın (Kolay)
        else {
            // Fallback to difficulty based
