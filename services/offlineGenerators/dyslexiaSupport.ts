...
export const generateOfflineAttentionFocus = async (options: GeneratorOptions): Promise<AttentionFocusData[]> => {
    ...
    // Gelişmiş Özellik Havuzu (Offline mantığı için sabit veri seti)
    ...
    const itemPool: ItemPoolItem[] = [
        ...
        // ... (Diğer öğeler helpers'a taşınabilir veya burada tutulabilir, kısalık için örnek)
        { 
            name: 'Çilek', category: 'Meyve', 
            features: ['üzerinde küçük noktalar vardır', 'yazın yetişir'], 
            negations: ['ekşi değildir'] 
        }
    ];
    // Not: Gerçek uygulamada bu havuz helpers.ts içinde genişletilmelidir.

    return Array.from({ length: worksheetCount }, () => {
        const puzzles = Array.from({ length: count }, () => {
            const target = getRandomItems(itemPool, 1)[0] || { name: 'Elma', category: 'Meyve', features: ['kırmızı'], negations: [] };
            
            // Basit Riddle Oluşturma
            const feature = getRandomItems(target.features, 1)[0];
            const riddle = `Aradığım şey bir ${target.category}. ${feature}.`;
            
            return {
                riddle,
                boxes: [{ title: 'Kutu A', items: shuffle([target.name, 'Armut', 'Muz']) }, { title: 'Kutu B', items: ['Kiraz', 'Karpuz', 'Kavun'] }],
                options: shuffle([target.name, 'Armut', 'Kiraz']),
                answer: target.name
            };
        });
        ...
        return {
            ...
            instruction: "Verilen özellikleri dikkatlice oku ve doğru cevabı bul.",
            pedagogicalNote: "İşitsel/Sözel dikkati sürdürme ve eleme yöntemiyle problem çözme.",
...
// 4. Attention Development (Offline Logic)
export const generateOfflineAttentionDevelopment = async (options: GeneratorOptions): Promise<AttentionDevelopmentData[]> => {
    ...
    return Array.from({ length: worksheetCount }, () => {
        const puzzles = Array.from({ length: count }, () => {
            ...
            const distractors = [answer + 1, answer - 1, getRandomInt(10, 99)];
            
            return {
                riddle: `Bu sayı ${answer > 50 ? '50\'den büyük' : '50\'den küçük'} bir sayıdır.`,
                boxes: [
                    { label: 'Sol', numbers: shuffle([answer, ...distractors]) },
                    { label: 'Sağ', numbers: Array.from({ length: 4 }, () => getRandomInt(10, 99)) }
                ],
                options: shuffle([answer.toString(), distractors[0].toString()]),
        ...
        return {
            ...
            instruction: 'İpuçlarını takip et ve doğru cevabı bul.',
            pedagogicalNote: 'İşleyen bellek (working memory).',
            imagePrompt: 'Dikkat',
            puzzles
        };
    });
...
export const generateOfflineRapidNaming = async (options: GeneratorOptions): Promise<RapidNamingData[]> => {
    const { worksheetCount, type } = options; 
