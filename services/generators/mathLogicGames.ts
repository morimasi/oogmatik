...
export const generateNumberCapsuleFromAI = async(options: OfflineGeneratorOptions): Promise<NumberCapsuleData[]> => {
    const { difficulty, worksheetCount } = options;
...
export const generateRomanNumeralConnectFromAI = async(options: OfflineGeneratorOptions): Promise<RomanNumeralConnectData[]> => {
    const { difficulty, worksheetCount } = options;
...
export const generateRomanNumeralStarHuntFromAI = async(options: OfflineGeneratorOptions): Promise<RomanNumeralStarHuntData[]> => {
    const { difficulty, worksheetCount } = options;
...
export const generateRoundingConnectFromAI = async(options: OfflineGeneratorOptions): Promise<RoundingConnectData[]> => {
    const { difficulty, worksheetCount } = options;
...
export const generateRomanNumeralMultiplicationFromAI = async(options: OfflineGeneratorOptions): Promise<RomanNumeralMultiplicationData[]> => {
    const { difficulty, worksheetCount } = options;
...
export const generateSudoku6x6ShadedFromAI = async(options: OfflineGeneratorOptions): Promise<Sudoku6x6ShadedData[]> => {
    const { difficulty, worksheetCount } = options;
...
export const generateKendokuFromAI = async(options: OfflineGeneratorOptions): Promise<KendokuData[]> => {
    const { difficulty, worksheetCount } = options;
    const prompt = `Create a 4x4 Kendoku (Calcudoku) puzzle appropriate for difficulty level "${difficulty}". Generate a worksheet with 1 puzzle. Provide the size, an empty grid, and a list of 'cages'. Each cage specifies the cells it contains, the arithmetic operation (+, −, ×, ÷), and the target number. 
    Her seferinde tamamen yeni, benzersiz ve daha önce ürettiklerinden farklı bir içerik oluştur. Başlıklar, istemler ve içerikler çocuklar için eğlenceli, ilgi çekici ve yaratıcı olsun.
    Create ${worksheetCount} unique worksheets based on these rules and return them in a JSON array.`;
    const singleSchema = {
        type: Type.OBJECT,
...
export const generateDivisionPyramidFromAI = async(options: OfflineGeneratorOptions): Promise<DivisionPyramidData[]> => {
    const { difficulty, worksheetCount } = options;
    const prompt = `Create a division number pyramid puzzle appropriate for difficulty level "${difficulty}". Generate a worksheet with 2 pyramids. Each has 4-5 rows. A number in a cell is the result of dividing the number above it by the one to its left or right. Some cells should be empty (null). 
    Her seferinde tamamen yeni, benzersiz ve daha önce ürettiklerinden farklı bir içerik oluştur. Başlıklar, istemler ve içerikler çocuklar için eğlenceli, ilgi çekici ve yaratıcı olsun.
    Create ${worksheetCount} unique worksheets based on these rules and return them in a JSON array.`;
    const singleSchema = {
        type: Type.OBJECT,
...
export const generateMultiplicationPyramidFromAI = async(options: OfflineGeneratorOptions): Promise<MultiplicationPyramidData[]> => {
    const { difficulty, worksheetCount } = options;
    const prompt = `Create a multiplication number pyramid puzzle appropriate for difficulty level "${difficulty}". Generate a worksheet with 2 pyramids. Each has 4 rows. A number in a cell (above the base) is the product of the two cells directly below it. Some cells should be empty (null). 
    Her seferinde tamamen yeni, benzersiz ve daha önce ürettiklerinden farklı bir içerik oluştur. Başlıklar, istemler ve içerikler çocuklar için eğlenceli, ilgi çekici ve yaratıcı olsun.
    Create ${worksheetCount} unique worksheets based on these rules and return them in a JSON array.`;
    const singleSchema = {
        type: Type.OBJECT,
...
export const generateOperationSquareSubtractionFromAI = async(options: OfflineGeneratorOptions): Promise<OperationSquareSubtractionData[]> => {
    const { difficulty, worksheetCount } = options;
    const prompt = `Create a 3x3 operation square puzzle using subtraction, appropriate for difficulty level "${difficulty}". Fill a grid with numbers and operation signs ('-', '=') such that the rows and columns form correct equations. Some numbers should be missing (represented by null). Generate a worksheet with 2 such puzzles. 
    Her seferinde tamamen yeni, benzersiz ve daha önce ürettiklerinden farklı bir içerik oluştur. Başlıklar, istemler ve içerikler çocuklar için eğlenceli, ilgi çekici ve yaratıcı olsun.
    Create ${worksheetCount} unique worksheets based on these rules and return them in a JSON array.`;
    const singleSchema = {
        type: Type.OBJECT,
...
export const generateMultiplicationWheelFromAI = async(options: OfflineGeneratorOptions): Promise<MultiplicationWheelData[]> => {
    const { difficulty, worksheetCount } = options;
    const prompt = `Create a multiplication wheel puzzle appropriate for difficulty level "${difficulty}". Generate a worksheet with 2 puzzles. Each wheel has a center number (the multiplier). There are 8 outer numbers to be multiplied by the center number to get the inner results. Some outer numbers or inner results should be missing (null). 
    Her seferinde tamamen yeni, benzersiz ve daha önce ürettiklerinden farklı bir içerik oluştur. Başlıklar, istemler ve içerikler çocuklar için eğlenceli, ilgi çekici ve yaratıcı olsun.
    Create ${worksheetCount} unique worksheets based on these rules and return them in a JSON array.`;
    const singleSchema = {
        type: Type.OBJECT,
...
export const generateTargetNumberFromAI = async (options: OfflineGeneratorOptions): Promise<TargetNumberData[]> => {
    const { difficulty, worksheetCount } = options;
    const prompt = `Create a 'Target Number' puzzle appropriate for difficulty level "${difficulty}". Generate a worksheet with 3 puzzles. For each puzzle, provide a target number and 4-5 given numbers. The user should use the given numbers and basic arithmetic operations (+, -, *, /) to reach the target. 
    Her seferinde tamamen yeni, benzersiz ve daha önce ürettiklerinden farklı bir içerik oluştur. Başlıklar, istemler ve içerikler çocuklar için eğlenceli, ilgi çekici ve yaratıcı olsun.
    Create ${worksheetCount} unique worksheets based on these rules and return them in a JSON array.`;
    const singleSchema = {
        type: Type.OBJECT,
...
export const generateOperationSquareMultDivFromAI = async(options: OfflineGeneratorOptions): Promise<OperationSquareMultDivData[]> => {
    const { difficulty, worksheetCount } = options;
    const prompt = `Create a 3x3 operation square puzzle using multiplication and division, appropriate for difficulty level "${difficulty}". Fill a grid with numbers and signs ('×', '÷', '=') to form correct equations. Some numbers should be missing (null). Generate a worksheet with 2 puzzles. 
    Her seferinde tamamen yeni, benzersiz ve daha önce ürettiklerinden farklı bir içerik oluştur. Başlıklar, istemler ve içerikler çocuklar için eğlenceli, ilgi çekici ve yaratıcı olsun.
    Create ${worksheetCount} unique worksheets based on these rules and return them in a JSON array.`;
    const singleSchema = {
        type: Type.OBJECT,
...
export const generateShapeSudokuFromAI = async(options: OfflineGeneratorOptions): Promise<ShapeSudokuData[]> => {
    const { difficulty, worksheetCount } = options;
    const prompt = `Create a 6x6 Shape Sudoku puzzle appropriate for difficulty level "${difficulty}". Provide a grid with some pre-filled shapes (null for empty). Also, provide the list of 6 shapes to be used. The user must fill the grid following Sudoku rules. 
    Her seferinde tamamen yeni, benzersiz ve daha önce ürettiklerinden farklı bir içerik oluştur. Başlıklar, istemler ve içerikler çocuklar için eğlenceli, ilgi çekici ve yaratıcı olsun.
    Create ${worksheetCount} unique worksheets based on these rules and return them in a JSON array.`;
    const singleSchema = {
        type: Type.OBJECT,
...
export const generateFutoshikiLengthFromAI = async(options: OfflineGeneratorOptions): Promise<FutoshikiLengthData[]> => {
    const { difficulty, worksheetCount } = options;
