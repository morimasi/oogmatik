...
export const generateLogicErrorHunterFromAI = async (
  ...
  let degreeInstruction =
    'Çok açık, herkesin kolayca fark edebileceği günlük mantık hataları kullan (Örn: Suyu çatalla içti, Güneş gece doğdu).';
  if (absurdityDegree === 'minimal') {
    degreeInstruction =
      'Daha ince, dikkat gerektiren ve okuduğunu anlamaya dayalı ufak mantık hataları kullan (Örn: Havuza atladığında elbisesi kupkuruydu, çok terlediği için kazak giydi).';
  } else if (absurdityDegree === 'surreal') {
    degreeInstruction =
      'Gerçeküstü, fantastik düzeyde absürt ve komik hatalar içeren ancak yapısal olarak doğru olan cümleler kur (Örn: Uçan inekler trafik sıkışıklığına yol açtı).';
  }
