...
const AppContent = () => {
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--app-font-size-scale', uiSettings.fontSizeScale.toString());
    root.style.setProperty('--app-line-height', uiSettings.lineHeight.toString());
    root.style.setProperty(
      '--app-letter-spacing',
      uiSettings.letterSpacing === 'wide' ? '0.05em' : 'normal'
    );
    root.style.setProperty('--app-saturation', `${uiSettings.saturation}%`);
    root.style.setProperty('--app-premium-intensity', `${uiSettings.premiumIntensity}%`);
    root.style.setProperty('--app-contrast-offset', `${uiSettings.contrastLevel}%`);
    root.style.setProperty('--app-padding-multiplier', uiSettings.compactMode ? '0.75' : '1');
    ...
    if (uiSettings.letterSpacing === 'wide') {
      ...
      root.classList.remove('letter-spacing-wide');
    }

    // Apply filters to body
    document.body.style.fontFamily = uiSettings.fontFamily;
    document.body.style.filter = `saturate(${uiSettings.saturation}%) contrast(${100 + uiSettings.contrastLevel}%)`;

    // Dynamic blur/glass updates
    root.style.setProperty('--blur-premium', `${(uiSettings.premiumIntensity / 100) * 25}px`);
    root.style.setProperty(
      '--surface-glass-opacity',
      `${(uiSettings.premiumIntensity / 100) * 0.1}`
    );
