 ...
 */
export const checkNetworkConnection = async (checkUrl: string = 'https://www.google.com/favicon.ico'): Promise<boolean> => {
    try {
...
export const batchRetry = async <T>(
    ...
    const results = await Promise.allSettled(
        tasks.map(task => retryWithBackoff(task, options))
    );
